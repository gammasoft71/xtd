  ...
  namespace forms {
    ...
    public:
      progress_dialog() {
        ...
        maximize_box(false);
        control_box(false);
        form_closing += [this](control& sender, form_closing_event_args& e) {
          e.cancel(true);
          visible(false);
        };
      ...
      virtual bool show_cancel() const {return show_cancel_;}
      virtual progress_dialog& show_cancel(bool value) {
        if (show_cancel_ != value) {
          show_cancel_ = value;
          do_layout();
        }
        return *this;
      ...
      virtual std::string message() const {return message_.text();}
      virtual progress_dialog& message(const std::string& value) {
        if (message_.text() != value) {
          message_.text(value);
          do_layout();
        }
        return *this;
    ...
    protected:
      ...
      virtual void do_layout() {
        ...
        progress_bar_.width(show_cancel_ ? client_size_.width() - 30 - button_cancel_.width()  : client_size_.width() - 20);
        
        close_box(show_cancel_);
        button_cancel_.visible( show_cancel_);
...
int main() {
  ...
  button btn;
   ...
   btn.text("Click me");
   btn.click += [&] {
     //dialog_result result = progress.show_dialog();
     //cdebug << format("result = {}", result) << std::endl;
     progress.show();
  };
