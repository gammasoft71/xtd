...
namespace xtd::forms::style_sheets::tests {
  class test_class_(style_sheet_kde_dark_tests) {
    void test_method_(theme) {
      auto styles = style_sheet::style_sheets();
      auto style = styles.find("KDE (dark)")->second;
      assert::are_equal("KDE (dark)", style.theme().name(), csf_);
      assert::are_equal("Contains KDE (dark) theme.", style.theme().description(), csf_);
