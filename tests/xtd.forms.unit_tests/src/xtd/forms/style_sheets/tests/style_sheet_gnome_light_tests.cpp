...
namespace xtd::forms::style_sheets::tests {
  class test_class_(style_sheet_gnome_light_tests) {
    auto test_method_(theme) {
      auto styles = style_sheet::style_sheets();
      auto style = styles.find("GNOME (light)")->second;
      assert::are_equal("GNOME (light)", style.theme().name());
      assert::are_equal("Contains GNOME (light) theme.", style.theme().description());
