...
namespace unit_tests {
  ...
  public:
    ...
    void test_method_(create_with_specified_all_color) {
      border_color b(color_property {color::red});
      assert::are_equal(color::red, b.all().color(), csf_);
    }

    void test_method_(create_with_specified_left_top_right_bottom_color) {
      border_color b(color_property {color::red}, color_property {color::green}, color_property {color::blue}, color_property {color::yellow});
