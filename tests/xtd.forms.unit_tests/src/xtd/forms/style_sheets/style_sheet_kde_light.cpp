...
namespace unit_tests {
  class test_class_(test_style_sheet_kde_light) {
  public:
    void test_method_(theme) {
      auto styles = style_sheet::style_sheets();
      auto style = styles.find("KDE (light)")->second;
      assert::are_equal("KDE (light)", style.theme().name().text(), csf_);
      assert::are_equal("Contains KDE (light) theme.", style.theme().description().text(), csf_);
