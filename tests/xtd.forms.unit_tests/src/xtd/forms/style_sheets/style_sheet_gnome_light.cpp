...
namespace unit_tests {
  class test_class_(test_style_sheet_gnome_dark) {
  public:
    void test_method_(theme) {
      auto styles = style_sheet::style_sheets();
      auto style = styles.find("GNOME (light)")->second;
      assert::are_equal("GNOME (light)", style.theme().name().text(), csf_);
      assert::are_equal("Contains GNOME (light) theme.", style.theme().description().text(), csf_);
