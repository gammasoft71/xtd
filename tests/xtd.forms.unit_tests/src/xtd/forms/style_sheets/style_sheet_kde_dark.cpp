...
namespace unit_tests {
  class test_class_(test_style_sheet_kde_dark) {
  public:
    void test_method_(theme) {
      auto styles = style_sheet::style_sheets();
      auto style = styles.find("KDE (dark)")->second;
      assert::are_equal("KDE (dark)", style.theme().name().text(), csf_);
      assert::are_equal("Contains KDE (dark) theme.", style.theme().description().text(), csf_);
