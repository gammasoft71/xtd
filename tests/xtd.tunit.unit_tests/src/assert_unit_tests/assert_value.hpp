...
namespace assert_unit_tests {
  ...
  void assert_value(const std::string& name, const expected_t& expected, const actual_t& actual, const xtd::diagnostics::source_location& location = xtd::diagnostics::source_location::current()) {
    if (actual == expected) {
      if (xtd::environment::get_command_line_args().contains("--verbose")) std::cout << xtd::foreground_color(xtd::console_color::dark_green) << "  SUCCEED " << xtd::reset_color() << name << std::endl;
    } else {
      ...
      std::cout << "      " << location.file_name() << ":" << location.line() << std::endl << std::endl;
