...
namespace unit_tests {
  ...
  public:
    void test_method_(test_case_succeed) {
      int i = 42;
      xtd::tunit::assert::is_not_zero(i, xtd::tunit::line_info {"test_assert_is_not_zero_succeed.cpp", 9});
    }
  };
}
...
void test_(test_assert_is_not_zero_succeed, test_output) {
  ...
  assert_value_("Start 1 test from 1 test case\n"
                "  Start 1 test from test_assert_is_not_zero_succeed\n"
                "    PASSED test_case_succeed\n"
                "  End 1 test from test_assert_is_not_zero_succeed\n"
                "\n"
                "  Summary :\n"
                "    PASSED 1 test.\n"
