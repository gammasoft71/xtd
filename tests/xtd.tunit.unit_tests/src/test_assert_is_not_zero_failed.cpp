...
namespace unit_tests {
  ...
  public:
    void test_method_(test_case_failed) {
      int i = 0;
      xtd::tunit::assert::is_not_zero(i, xtd::tunit::line_info {"test_assert_is_not_zero_failed.cpp", 9});
    }
  };
}
...
void test_(test_assert_is_not_zero_failed, test_output) {
  ...
  assert_value_("Start 1 test from 1 test case\n"
                "  Start 1 test from test_assert_is_not_zero_failed\n"
                "    FAILED  test_case_failed\n"
                "\n"
                "Expected: not zero\n"
                "But was:  0\n"
                "error: test_assert_is_not_zero_failed.cpp:9\n"
                "\n"
                "  End 1 test from test_assert_is_not_zero_failed\n"
                "\n"
                "  Summary :\n"
