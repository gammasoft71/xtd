...
namespace unit_tests {
  ...
  public:
    void test_method_(test_case_succeed) {
      int a = 24;
      int& b = a;
      xtd::tunit::assert::are_same(b, a, xtd::tunit::line_info {"test_assert_are_same_succeed.cpp", 10});
    }
  };
}
...
void test_(test_assert_are_same_succeed, test_output) {
  ...
  assert_value_("Start 1 test from 1 test case\n"
                "  Start 1 test from test_assert_are_same_succeed\n"
                "    SUCCEED test_case_succeed\n"
                "  End 1 test from test_assert_are_same_succeed\n"
                "\n"
                "  Summary :\n"
                "    SUCCEED 1 test.\n"
