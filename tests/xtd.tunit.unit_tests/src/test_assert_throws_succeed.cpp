...
namespace unit_tests {
  ...
  public:
    void test_method_(test_case_succeed) {
      ...
      xtd::tunit::assert::throws<std::out_of_range>([&] {v.at(5);}, xtd::tunit::line_info {"test_assert_does_throws_failed.cpp", 10});
    }
  };
}
...
void test_(test_assert_throws_succeed, test_output) {
  ...
  assert_value_("Start 1 test from 1 test case\n"
                "  Start 1 test from test_assert_throws_succeed\n"
                "    PASSED test_case_succeed\n"
                "  End 1 test from test_assert_throws_succeed\n"
                "\n"
                "  Summary :\n"
                "    PASSED 1 test.\n"
