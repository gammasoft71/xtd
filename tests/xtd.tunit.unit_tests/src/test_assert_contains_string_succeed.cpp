...
namespace unit_tests {
  ...
  public:
    void test_method_(test_case_succeed) {
      std::string s = "string";
      xtd::tunit::assert::contains('i', s, xtd::tunit::line_info {"test_assert_contains_string_succeed.cpp", 10});
    }
  };
}
...
void test_(test_assert_contains_string_succeed, test_output) {
  ...
  assert_value_("Start 1 test from 1 test case\n"
                "  Start 1 test from test_assert_contains_string_succeed\n"
                "    SUCCEED test_case_succeed\n"
                "  End 1 test from test_assert_contains_string_succeed\n"
