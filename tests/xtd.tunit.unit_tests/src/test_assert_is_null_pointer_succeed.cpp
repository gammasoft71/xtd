...
namespace unit_tests {
  ...
  public:
    void test_method_(test_case_succeed) {
      int* p = nullptr;
      xtd::tunit::assert::is_null(p, xtd::tunit::line_info {"test_assert_is_null_pointer_succeed.cpp", 9});
    }
  };
}
...
void test_(test_assert_is_null_pointer_succeed, test_output) {
  ...
  assert_value_("Start 1 test from 1 test case\n"
                "  Start 1 test from test_assert_is_null_pointer_succeed\n"
                "    SUCCEED test_case_succeed\n"
                "  End 1 test from test_assert_is_null_pointer_succeed\n"
                "\n"
                "  Summary :\n"
                "    SUCCEED 1 test.\n"
