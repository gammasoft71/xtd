...
void test_(assert_is_not_null_unique_ptr_succeed_tests, test_output) {
  ...
  xtd::tunit::settings::default_settings().filter_tests("assert_is_not_null_unique_ptr_succeed_tests.*");
  std::stringstream ss;
  xtd::tunit::unit_test(std::make_unique<assert_unit_tests::unit_tests_event_listener>(ss)).run();
  assert_value_("Start 1 test from 1 test case\n"
    "  SUCCEED assert_is_not_null_unique_ptr_succeed_tests.test_case_succeed\n"
