...
namespace xtd::tunit::tests {
  ...
  public:
    auto test_method_(test_case_failed) {
      ...
      xtd::tunit::assert::throws_any([&] {[[maybe_unused]] auto __ = a[2];});
    }
  };
  
  void test_(assert_throws_any_failed_tests, test_output) {
    auto [output, result] = run_test_("xtd::tunit::tests::assert_throws_any_failed_tests.*");
    assert_value_("Start 1 test from 1 test case\n"
                  "   FAILED xtd::tunit::tests::assert_throws_any_failed_tests.test_case_failed\n"
                  "    Expected: <exception>\n"
                  "    But was:  <nothing>\n"
