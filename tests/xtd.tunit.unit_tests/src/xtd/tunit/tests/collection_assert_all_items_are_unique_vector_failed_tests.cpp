...
void test_(collection_assert_all_items_are_unique_vector_failed_tests, test_output) {
  ...
  xtd::tunit::settings::default_settings().filter_tests("collection_assert_all_items_are_unique_vector_failed_tests.*");
  std::stringstream ss;
  xtd::tunit::unit_test(std::make_unique<assert_unit_tests::unit_tests_event_listener>(ss)).run();
  assert_value_("Start 1 test from 1 test case\n"
    "  FAILED  collection_assert_all_items_are_unique_vector_failed_tests.test_case_failed\n"
