...
void test_(valid_are_equal_double_with_tolerence_succeed_tests, test_output) {
  ...
  xtd::tunit::settings::default_settings().filter_tests("valid_are_equal_double_with_tolerence_succeed_tests.*");
  std::stringstream ss;
  xtd::tunit::unit_test(std::make_unique<assert_unit_tests::unit_tests_event_listener>(ss)).run();
  assert_value_("Start 1 test from 1 test case\n"
    "  SUCCEED valid_are_equal_double_with_tolerence_succeed_tests.test_case_succeed\n"
