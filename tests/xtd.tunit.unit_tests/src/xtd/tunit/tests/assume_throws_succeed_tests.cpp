...
namespace xtd::tunit::tests {
  ...
  public:
    auto test_method_(test_case_succeed) {
      ...
      xtd::tunit::assume::throws<xtd::index_out_of_range_exception>([&] {[[maybe_unused]] auto __ = a[5];});
    }
  };
  
  void test_(assume_throws_succeed_tests, test_output) {
    auto [output, result] = run_test_("xtd::tunit::tests::assume_throws_succeed_tests.*");
    assert_value_("Start 1 test from 1 test case\n"
                  "  SUCCEED xtd::tunit::tests::assume_throws_succeed_tests.test_case_succeed\n"
