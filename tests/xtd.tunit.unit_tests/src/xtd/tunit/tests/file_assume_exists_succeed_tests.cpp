...
namespace xtd::tunit::tests {
  ...
  public:
    auto test_method_(test_case_succeed) {
      ...
      file_assume::exists(xtd::io::path::combine(xtd::io::path::get_temp_path(), "test_tunit_file.txt"));
  ...
  void test_(file_assume_exists_succeed_tests, test_output) {
    auto [output, result] = run_test_("xtd::tunit::tests::file_assume_exists_succeed_tests.*");
    assert_value_("Start 1 test from 1 test case\n"
                  "  SUCCEED xtd::tunit::tests::file_assume_exists_succeed_tests.test_case_succeed\n"
