...
namespace xtd {
  template<>
  std::wstring to_string(const my_object& value, const std::wstring& fmt, const std::locale& loc) {
...
namespace unit_tests {
  ...
  public:
    void test_method_(format_my_object_with_default_argument) {
      assert::are_equal(L"[42 24]", xtd::strings::format(L"{0}", my_object(42, 24)));
    }
    
    void test_method_(format_my_object_with_left_alignment) {
      assert::are_equal(L"   [42 24]", strings::format(L"{0,10}", my_object(42, 24)));
    }
    ...
    void test_method_(format_my_object_with_zero_alignment) {
      assert::are_equal(L"[42 24]", strings::format(L"{0,0}", my_object(42, 24)));
    }
    ...
    void test_method_(format_my_object_with_invalid_argument) {
      assert::throws<std::invalid_argument>([]{xtd::strings::format(L"{0:, }", my_object(42, 24));});
    }
  };
}
