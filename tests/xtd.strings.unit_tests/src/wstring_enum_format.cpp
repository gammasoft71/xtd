...
std::wostream& operator<<(std::wostream& os, day n) {
  switch (n) {
...
namespace unit_tests {
  ...
  public:
    void test_method_(format_sunday_with_default_argument) {
      assert::are_equal(L"sunday", xtd::strings::format(L"{0}", sunday));
    }
    
    void test_method_(format_monday_with_left_alignment) {
      assert::are_equal(L"    monday", strings::format(L"{0,10}", monday));
    }
    
    void test_method_(format_tuesday_with_right_alignment) {
      assert::are_equal(L"tuesday   ", strings::format(L"{0, -10}", tuesday));
    }
    
    void test_method_(format_wednesday_with_binary_argument) {
      assert::are_equal(L"11", xtd::strings::format(L"{0:b}", wednesday));
    }
    
    void test_method_(format_thursday_with_decimal_argument) {
      assert::are_equal(L"4", xtd::strings::format(L"{0:d}", thursday));
    }
    
    void test_method_(format_friday_with_general_argument) {
      assert::are_equal(L"friday", xtd::strings::format(L"{0:g}", friday));
    }
    
    void test_method_(format_saturday_with_octal_argument) {
      assert::are_equal(L"6", xtd::strings::format(L"{0:o}", saturday));
    }
    
    void test_method_(format_sunday_with_hexadecimal_argument) {
      assert::are_equal(L"0", xtd::strings::format(L"{0:x}", sunday));
    }
    
    void test_method_(format_tuesday_with_invalid_argument) {
      assert::throws<std::invalid_argument>([]{xtd::strings::format(L"{0:e}", tuesday);});
    }
  };
}
