...
namespace unit_tests {
  ...
  public:
    void test_method_(format_location_with_default_argument) {
      assert::are_equal(L"(42, 24)", xtd::strings::format(L"{0}", ::size {42, 24}));
    }
    
    void test_method_(format_location_with_left_alignment) {
      assert::are_equal(L"  (42, 24)", strings::format(L"{0,10}", ::size {42, 24}));
    }
    ...
    void test_method_(format_location_with_zero_alignment) {
      assert::are_equal(L"(42, 24)", strings::format(L"{0,0}", ::size {42, 24}));
    }
