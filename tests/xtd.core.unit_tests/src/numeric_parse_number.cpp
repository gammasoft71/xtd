...
namespace unit_tests {
  ...
  public:
    ...
    void test_method_(parse_styles_number_with_two_decimal_point) {
      assert::throws<argument_exception>([]{xtd::parse<Value>("4.2.0", number_styles::number);}, csf_);
    }
    ...
    void test_method_(parse_styles_number_with_two_thousands) {
      if (!std::is_same<Value, int8_t>::value && !std::is_same<Value, char>::value && !std::is_same<Value, unsigned char>::value)
        assert::throws<argument_exception>([]{xtd::parse<Value>("1,,234", number_styles::number);}, csf_);
    }
    
    void test_method_(parse_styles_number_with_exponent) {
      assert::throws<argument_exception>([]{xtd::parse<Value>("4E+01", number_styles::number);}, csf_);
    }
    ...
    void test_method_(parse_styles_number_with_binary_specifier) {
      assert::throws<argument_exception>([]{xtd::parse<Value>("0b101010", number_styles::number);}, csf_);
    }
    ...
    void test_method_(parse_styles_number_with_hexa_specifier) {
      assert::throws<argument_exception>([]{xtd::parse<Value>("0x2A", number_styles::number);}, csf_);
    }
    
    void test_method_(parse_styles_number_invalid) {
      assert::throws<argument_exception>([]{xtd::parse<Value>("z42", number_styles::number);}, csf_);
    }
  };
}
