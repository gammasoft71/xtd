...
namespace unit_tests {
  ...
  public:
    void test_method_(format_with_default_argument) {
      assert::are_equal("1.02:03:32:024000500", xtd::ustring::format("{0}", 26h + 3min + 32s + 24ms + 500ns));
    }
    
    void test_method_(format_with_default_argument_without_days_milliseconds_and_nanoseconds) {
      assert::are_equal("02:03:32", xtd::ustring::format("{0}", 2h + 3min + 32s));
    }
    ...
    void test_method_(format_with_left_alignment) {
      assert::are_equal("  02:03:32", ustring::format("{0,10}", 2h + 3min + 32s));
    }
    ...
    void test_method_(format_with_left_alignment_to_zero) {
      assert::are_equal("02:03:32", ustring::format("{0,0}", 2h + 3min + 32s));
    }
    ...
    void test_method_(format_with_alignment_invalid) {
      assert::throws<format_exception>([]{ustring::format("{0,a}",2h + 3min + 32s);});
    }

    void test_method_(format_with_invalid_argument) {
      assert::throws<xtd::format_exception>([]{xtd::ustring::format("{0:e}", 2h + 3min + 32s);});
    }

    void test_method_(format_with_constant_argument) {
      assert::are_equal("1.02:03:32:024000500", xtd::ustring::format("{0:c}", 26h + 3min + 32s + 24ms + 500ns));
    }
    
    void test_method_(format_with_constant_argument_without_days_milliseconds_and_nanoseconds) {
      assert::are_equal("02:03:32", xtd::ustring::format("{0:c}", 2h + 3min + 32s));
    }

    void test_method_(format_with_alternative_days_argument) {
      assert::are_equal("1", xtd::ustring::format("{0:d}", 26h + 3min + 32s + 24ms + 500ns));
    }
    
    void test_method_(format_with_alternative_days_argument_without_days_milliseconds_and_nanoseconds) {
      assert::are_equal("0", xtd::ustring::format("{0:d}", 2h + 3min + 32s));
    }

    void test_method_(format_with_days_argument) {
      assert::are_equal("01", xtd::ustring::format("{0:D}", 26h + 3min + 32s + 24ms + 500ns));
    }
    
    void test_method_(format_with_days_argument_without_days_milliseconds_and_nanoseconds) {
      assert::are_equal("00", xtd::ustring::format("{0:D}", 2h + 3min + 32s));
    }
    ...
    void test_method_(format_with_alternative_full_argument_without_days_milliseconds_and_nanoseconds) {
      assert::are_equal("0:2:03:32:000000000", xtd::ustring::format("{0:f}", 2h + 3min + 32s));
    }
