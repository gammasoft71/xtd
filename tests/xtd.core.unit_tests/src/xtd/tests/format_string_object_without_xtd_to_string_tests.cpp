...
namespace xtd::tests {
  class test_class_(format_string_object_without_xtd_to_string_tests) {
    auto test_method_(format_location_with_default_argument) {
      assert::are_equal("(42, 24)", format("{0}", ::size_sof {42, 24}));
    }
    
    auto test_method_(format_location_with_left_alignment) {
      assert::are_equal("  (42, 24)", format("{0,10}", ::size_sof {42, 24}));
    }
    ...
    auto test_method_(format_location_with_zero_alignment) {
      assert::are_equal("(42, 24)", format("{0,0}", ::size_sof {42, 24}));
    }
    
    auto test_method_(format_location_with_invalid_argument) {
      assert::are_equal("(42, 24)", format("{0:G}", ::size_sof {42, 24}));
    }
  };
}
