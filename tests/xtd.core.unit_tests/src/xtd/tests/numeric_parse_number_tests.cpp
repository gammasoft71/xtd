...
namespace xtd::tests {
  ...
  class numeric_parse_string_number_tests : public test_class {
    ...
    auto test_method_(parse_styles_number_with_decimal_point) {
      assert::are_equal(static_cast<value_t>(42), xtd::parse<value_t>("42.0", number_styles::number));
    }
    
    auto test_method_(parse_styles_number_with_two_decimal_point) {
      assert::throws<format_exception>([] {xtd::parse<value_t>("4.2.0", number_styles::number);});
    }
    ...
    auto test_method_(parse_styles_number_with_thousands) {
      if (!std::is_same<value_t, sbyte>::value && !std::is_same<value_t, char>::value && !std::is_same<value_t, unsigned char>::value)
        assert::are_equal(static_cast<value_t>(1234), xtd::parse<value_t>("1,234", number_styles::number));
    }
    
    auto test_method_(parse_styles_number_with_two_thousands) {
      if (!std::is_same<value_t, sbyte>::value && !std::is_same<value_t, char>::value && !std::is_same<value_t, unsigned char>::value)
        assert::throws<format_exception>([] {xtd::parse<value_t>("1,,234", number_styles::number);});
    }
    
    auto test_method_(parse_styles_number_with_exponent) {
      assert::throws<format_exception>([] {xtd::parse<value_t>("4E+01", number_styles::number);});
    }
    ...
    auto test_method_(parse_styles_number_with_binary_specifier) {
      assert::throws<format_exception>([] {xtd::parse<value_t>("0b101010", number_styles::number);});
    }
    
    auto test_method_(parse_styles_number_with_octal_specifier) {
      assert::are_equal(static_cast<value_t>(52), xtd::parse<value_t>("052", number_styles::number));
    }
    
    auto test_method_(parse_styles_number_with_hexa_specifier) {
      assert::throws<format_exception>([] {xtd::parse<value_t>("0x2A", number_styles::number);});
    }
    
    auto test_method_(parse_styles_number_invalid) {
      assert::throws<format_exception>([] {xtd::parse<value_t>("z42", number_styles::number);});
    }
  };
}
