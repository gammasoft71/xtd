...
namespace xtd::tests {
  generic_test_class_(numeric_parse_string_number_tests, signed char, unsigned char, short, unsigned short, int, unsigned int, long, unsigned long, long long, unsigned long long, sbyte, int16, int32, int64, intptr, ptrdiff, usize, byte, uint16, uint32, uint64, uintptr) {
    ...
    auto test_method_(parse_styles_number_with_two_leading_plus_sign) {
      assert::throws<format_exception>([] {[[maybe_unused]] auto __ = xtd::parse<type_t>("++42", number_styles::number);});
    }
    ...
    auto test_method_(parse_styles_number_with_decimal_point) {
      assert::are_equal(static_cast<type_t>(42), xtd::parse<type_t>("42.0", number_styles::number));
    }
    
    auto test_method_(parse_styles_number_with_two_decimal_point) {
      assert::throws<format_exception>([] {[[maybe_unused]] auto __ = xtd::parse<type_t>("4.2.0", number_styles::number);});
    }
    ...
    auto test_method_(parse_styles_number_with_thousands) {
      if (!std::is_same<type_t, sbyte>::value && !std::is_same<type_t, char>::value && !std::is_same<type_t, unsigned char>::value)
        assert::are_equal(static_cast<type_t>(1234), xtd::parse<type_t>("1,234", number_styles::number));
    }
    
    auto test_method_(parse_styles_number_with_two_thousands) {
      if (!std::is_same<type_t, sbyte>::value && !std::is_same<type_t, char>::value && !std::is_same<type_t, unsigned char>::value)
        assert::throws<format_exception>([] {[[maybe_unused]] auto __ = xtd::parse<type_t>("1,,234", number_styles::number);});
    }
    
    auto test_method_(parse_styles_number_with_exponent) {
      assert::throws<format_exception>([] {[[maybe_unused]] auto __ = xtd::parse<type_t>("4E+01", number_styles::number);});
    }
    ...
    auto test_method_(parse_styles_number_with_binary_specifier) {
      assert::throws<format_exception>([] {[[maybe_unused]] auto __ = xtd::parse<type_t>("0b101010", number_styles::number);});
    }
    
    auto test_method_(parse_styles_number_with_octal_specifier) {
      assert::are_equal(static_cast<type_t>(52), xtd::parse<type_t>("052", number_styles::number));
    }
    
    auto test_method_(parse_styles_number_with_hexa_specifier) {
      assert::throws<format_exception>([] {[[maybe_unused]] auto __ = xtd::parse<type_t>("0x2A", number_styles::number);});
    }
    
    auto test_method_(parse_styles_number_invalid) {
      assert::throws<format_exception>([] {[[maybe_unused]] auto __ = xtd::parse<type_t>("z42", number_styles::number);});
    }
  };
}
