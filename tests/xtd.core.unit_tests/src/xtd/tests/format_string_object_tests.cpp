...
namespace xtd::tests {
  class test_class_(format_string_object_tests) {
    auto test_method_(format_point_with_default_argument) {
      assert::are_equal("(42, 24)", format("{0}", point {42, 24}));
    }
    
    auto test_method_(format_point_with_left_alignment) {
      assert::are_equal("  (42, 24)", format("{0,10}", point {42, 24}));
    }
    ...
    auto test_method_(format_point_with_zero_alignment) {
      assert::are_equal("(42, 24)", format("{0,0}", point {42, 24}));
    }
    
    auto test_method_(format_point_with_general_argument) {
      assert::are_equal("(42, 24)", format("{0:G}", point {42, 24}));
    }
    
    auto test_method_(format_point_with_label_argument) {
      assert::are_equal("(x=42, y=24)", format("{0:l}", point {42, 24}));
    }
    ...
    auto test_method_(format_point_with_invalid_argument) {
      assert::throws<xtd::format_exception>([] {[[maybe_unused]] auto __ = format("{0:, }", point {42, 24});});
    }
  };
}
