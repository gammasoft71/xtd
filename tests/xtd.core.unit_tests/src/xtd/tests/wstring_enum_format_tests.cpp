...
namespace xtd::tests {
  class test_class_(wstring_enum_format_tests) {
    void test_method_(format_sunday_with_default_argument) {
      assert::are_equal(L"sunday", xtd::string::format(L"{0}", sunday), csf_);
    }
    
    void test_method_(format_monday_with_left_alignment) {
      assert::are_equal(L"    monday", string::format(L"{0,10}", monday), csf_);
    }
    
    void test_method_(format_tuesday_with_right_alignment) {
      assert::are_equal(L"tuesday   ", string::format(L"{0, -10}", tuesday), csf_);
    }
    
    void test_method_(format_wednesday_with_binary_argument) {
      assert::are_equal(L"11", xtd::string::format(L"{0:b}", wednesday), csf_);
    }
    
    void test_method_(format_thursday_with_decimal_argument) {
      assert::are_equal(L"4", xtd::string::format(L"{0:d}", thursday), csf_);
    }
    
    void test_method_(format_friday_with_general_argument) {
      assert::are_equal(L"friday", xtd::string::format(L"{0:g}", friday), csf_);
    }
    
    void test_method_(format_saturday_with_octal_argument) {
      assert::are_equal(L"6", xtd::string::format(L"{0:o}", saturday), csf_);
    }
    
    void test_method_(format_sunday_with_hexadecimal_argument) {
      assert::are_equal(L"0", xtd::string::format(L"{0:x}", sunday), csf_);
    }
    
    void test_method_(format_tuesday_with_invalid_argument) {
      assert::throws<xtd::format_exception>([] {xtd::string::format(L"{0:e}", tuesday);}, csf_);
    }
  };
}
