...
namespace xtd::tests {
  class test_class_(format_wstring_date_time_tests) {
    ...
    void test_method_(format_date_time_local_with_std_tm_with_default_argument) {
      ...
      assert::are_equal(L"Wed Jan  2 03:04:05 2019", format(L"{0}", date_time {2019, 1, 2, 3, 4, 5}));
      #elif defined(_WIN32)
    ...
    void test_method_(format_date_time_local_with_left_alignment) {
      #if defined(__APPLE__)
      assert::are_equal(L"           Wed Jan  2 03:04:05 2019", format(L"{0, 35}", date_time {2019, 1, 2, 3, 4, 5}));
      #elif defined(_WIN32)
      assert::are_equal(L"                1/2/2019 3:04:05 AM", format(L"{0, 35}", date_time {2019, 1, 2, 3, 4, 5}));
      #else
      string_assert::starts_with("    Wed 02 Jan 2019 03:04:05 AM", format(L"{0, 35}", date_time {2019, 1, 2, 3, 4, 5}));
      #endif
    }
    ...
    void test_method_(format_date_time_local_with_left_to_zero) {
      #if defined(__APPLE__)
      assert::are_equal(L"Wed Jan  2 03:04:05 2019", format(L"{0,0}", date_time {2019, 1, 2, 3, 4, 5}));
      #elif defined(_WIN32)
      assert::are_equal(L"1/2/2019 3:04:05 AM", format(L"{0,0}", date_time {2019, 1, 2, 3, 4, 5}));
      #else
      string_assert::starts_with("Wed 02 Jan 2019 03:04:05 AM", format(L"{0,0}", date_time {2019, 1, 2, 3, 4, 5}));
      #endif
    }
    ...
    void test_method_(format_date_time_local_with_std_tm_with_time_zone_argument) {
      ...
      #else
      assert::are_equal(L"GMT", format(L"{0:Z}", date_time {2019, 1, 2, 3, 4, 5}.to_universal_time()));
      #endif
    }
  };
}
