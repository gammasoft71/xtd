...
template<> struct xtd::enum_register<day> {
  static auto values() noexcept {return xtd::enum_collection<day> {{day::sunday,  "sunday"}, {day::monday, "monday"}, {day::tuesday, "tuesday"}, {day::wednesday, "wednesday"}, {day::thursday, "thursday"}, {day::friday, "friday"}, {day::saturday, "saturday"}};}
};
...
namespace xtd::tests {
  class test_class_(format_u8string_enum_tests) {
    ...
    auto test_method_(format_tuesday_with_invalid_argument) {
      assert::throws<xtd::format_exception>([] {[[maybe_unused]] auto __ = format(u8"{0:e}", tuesday);});
    }
  };
}
