...
namespace xtd::tests {
  class test_class_(string_object_format_without_xtd_to_string_tests) {
    void test_method_(format_location_with_default_argument) {
      assert::throws<xtd::format_exception>([] {xtd::string::format("{0}", ::size_sof {42, 24});}, csf_);
    }
    
    void test_method_(format_location_with_left_alignment) {
      assert::throws<xtd::format_exception>([] {string::format("{0,10}", ::size_sof {42, 24});}, csf_);
    }
    ...
    void test_method_(format_location_with_zero_alignment) {
      assert::throws<xtd::format_exception>([] {string::format("{0,0}", ::size_sof {42, 24});}, csf_);
    }
    
    void test_method_(format_location_with_invalid_argument) {
      assert::throws<xtd::format_exception>([] {xtd::string::format("{0:G}", ::size_sof {42, 24});}, csf_);
    }
  };
}
