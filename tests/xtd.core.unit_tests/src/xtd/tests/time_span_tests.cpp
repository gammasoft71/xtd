...
namespace xtd::tests {
  ...
  class test_class_(time_span_suffixes_tests) {
    ...
    void test_method_(_ts_operator_on_on_char_ptr) {
      auto v = "6:12:14"_ts;
      assert::is_instance_of<time_span>(v);
    ...
    void test_method_(_ts_operator_on_on_char32_ptr) {
      auto v = U"6:12:14"_ts;
      assert::is_instance_of<time_span>(v);
      assert::are_equal(time_span {6, 12, 14}, v);
    }
    
    void test_method_(_ts_operator_on_on_wchar_ptr) {
