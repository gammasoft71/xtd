...
namespace xtd::tests {
  ...
  public:
    ...
    inline static void are_equal(value_t b, const array<bytes_t>& bytes, const xtd::diagnostics::stack_frame& stack_frame = xtd::diagnostics::stack_frame::current()) {are_equal(b, bytes, xtd::string::empty_string, stack_frame);}
    
    template<class value_t, class bytes_t>
    inline static void are_equal(value_t b, const array<bytes_t>& bytes, const std::string& message, const xtd::diagnostics::stack_frame& stack_frame = xtd::diagnostics::stack_frame::current()) {
      if (bytes.length() == 1 && bytes[0] == b)
    ...
    inline static void are_equal(value1_t b1, value2_t b2, const array<bytes_t>& bytes, const xtd::diagnostics::stack_frame& stack_frame = xtd::diagnostics::stack_frame::current()) {are_equal(b1, b2, bytes, xtd::string::empty_string, stack_frame);}
    
    template<class value1_t, class value2_t, class bytes_t>
    inline static void are_equal(value1_t b1, value2_t b2, const array<bytes_t>& bytes, const std::string& message, const xtd::diagnostics::stack_frame& stack_frame = xtd::diagnostics::stack_frame::current()) {
      if (bytes.length() == 2 && ((is_little_endian() && bytes[0] == b1 && bytes[1] == b2) || (bytes[0] == b2 && bytes[1] == b1)))
        base_assert::succeed(message, stack_frame);
      else
    ...
    inline static void are_equal(value1_t b1, value2_t b2, value3_t b3, value4_t b4, const array<bytes_t>& bytes, const xtd::diagnostics::stack_frame& stack_frame = xtd::diagnostics::stack_frame::current()) {are_equal(b1, b2, b3, b4, bytes, xtd::string::empty_string, stack_frame);}
    
    template<class value1_t, class value2_t, class value3_t, class value4_t, class bytes_t>
    inline static void are_equal(value1_t b1, value2_t b2, value3_t b3, value4_t b4, const array<bytes_t>& bytes, const std::string& message, const xtd::diagnostics::stack_frame& stack_frame = xtd::diagnostics::stack_frame::current()) {
      if (bytes.length() == 4 && ((is_little_endian() && bytes[0] == b1 && bytes[1] == b2 && bytes[2] == b3 && bytes[3] == b4) || (bytes[0] == b4 && bytes[1] == b3 && bytes[2] == b2 && bytes[3] == b1)))
        base_assert::succeed(message, stack_frame);
      else
    ...
    template<class value1_t, class value2_t, class value3_t, class value4_t, class value5_t, class value6_t, class value7_t, class value8_t, class bytes_t>
    inline static void are_equal(value1_t b1, value2_t b2, value3_t b3, value4_t b4, value5_t b5, value6_t b6, value7_t b7, value8_t b8, const array<bytes_t>& bytes, const std::string& message, const xtd::diagnostics::stack_frame& stack_frame = xtd::diagnostics::stack_frame::current()) {
      if (bytes.length() == 8 && ((is_little_endian() && bytes[0] == b1 && bytes[1] == b2 && bytes[2] == b3 && bytes[3] == b4 && bytes[4] == b5 && bytes[5] == b6 && bytes[6] == b7 && bytes[7] == b8) || (bytes[0] == b8 && bytes[1] == b7 && bytes[2] == b6 && bytes[3] == b5 && bytes[4] == b4 && bytes[5] == b3 && bytes[6] == b2 && bytes[7] == b1)))
        base_assert::succeed(message, stack_frame);
      else
