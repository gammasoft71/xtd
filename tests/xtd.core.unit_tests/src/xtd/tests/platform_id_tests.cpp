...
namespace xtd::tests {
  class test_class_(platform_id_tests) {
    ...
    void test_method_(mingw) {
      assert::are_equal(13, enum_object<>::to_int32(platform_id::mingw), csf_);
      assert::are_equal("mingw", enum_object<>::to_string(platform_id::mingw), csf_);
      assert::are_equal(platform_id::mingw, enum_object<>::parse<platform_id>("mingw"), csf_);
    }
    
    void test_method_(msys) {
      assert::are_equal(14, enum_object<>::to_int32(platform_id::msys), csf_);
