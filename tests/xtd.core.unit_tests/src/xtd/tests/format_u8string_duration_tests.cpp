...
namespace xtd::tests {
  class test_class_(format_u8string_duration_tests) {
    ...
    auto test_method_(format_without_format_argument_separator) {
      assert::throws<format_exception>([] {[[maybe_unused]] auto _ = string::format(u8"{0G}", std::chrono::hours {2} + std::chrono::minutes {3} + std::chrono::seconds {32});});
    }
    ...
    auto test_method_(format_with_alignment_invalid) {
      assert::throws<format_exception>([] {[[maybe_unused]] auto _ = string::format(u8"{0,a}", std::chrono::hours {2} + std::chrono::minutes {3} + std::chrono::seconds {32});});
    }
    
    auto test_method_(format_with_invalid_argument) {
      assert::throws<xtd::format_exception>([] {[[maybe_unused]] auto _ = format(u8"{0:e}", std::chrono::hours {2} + std::chrono::minutes {3} + std::chrono::seconds {32});});
    }
