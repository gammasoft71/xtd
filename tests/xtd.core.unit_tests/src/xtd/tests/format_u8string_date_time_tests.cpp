...
namespace xtd::tests {
  class test_class_(format_u8string_date_time_tests) {
    ...
    auto test_method_(format_with_none_and_alignment_invalid) {
      assert::throws<format_exception>([] {auto __ = format(u8"{,a}", date_time {2019, 1, 2, 3, 4, 5});});
      assert::throws<format_exception>([] {auto __ = format(u8"{, }", date_time {2019, 1, 2, 3, 4, 5});});
    }
    
    auto test_method_(format_with_sequence_number_and_alignment_invalid) {
      assert::throws<format_exception>([] {auto __ = format(u8"{0,a}", date_time {2019, 1, 2, 3, 4, 5});});
      assert::throws<format_exception>([] {auto __ = format(u8"{0, }", date_time {2019, 1, 2, 3, 4, 5});});
    }
