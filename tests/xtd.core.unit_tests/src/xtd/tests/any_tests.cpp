...
namespace xtd::tests {
  class test_class_(any_tests) {
    ...
    void test_method_(format_empty) {
      std::any a;
      assert::are_equal("{}", string::format("{}", a), csf_);
    }
    
    void test_method_(format_byte) {
      std::any a = byte(42);
      assert::are_equal("42", string::format("{}", a), csf_);
    }
    
    void test_method_(format_char) {
      std::any a = 'a';
      assert::are_equal("a", string::format("{}", a), csf_);
    }
    ...
    void test_method_(format_char_pointer) {
      std::any a = "string";
      assert::are_equal("string", string::format("{}", a), csf_);
    }
    
    void test_method_(format_double) {
      std::any a = double(.42);
      assert::are_equal("0.42", string::format("{}", a), csf_);
    }
    ...
    void test_method_(format_string8) {
      ...
      assert::are_equal(u8"string", string::format("{}", a), csf_);
#endif
    }
    
    void test_method_(format_string16) {
      std::any a = std::u16string {u"string"};
      assert::are_equal(u"string", string::format("{}", a), csf_);
    }
    ...
    void test_method_(format_ustring) {
      ...
      assert::are_equal("string", string::format("{}", a1), csf_);
      std::any a2 = u8"string"_s;
      assert::are_equal("string", string::format("{}", a2), csf_);
      std::any a3 = u"string"_s;
      assert::are_equal("string", string::format("{}", a3), csf_);
      std::any a4 = U"string"_s;
      assert::are_equal("string", string::format("{}", a4), csf_);
      std::any a5 = L"string"_s;
      assert::are_equal("string", string::format("{}", a5), csf_);
    }
    ...
    void test_method_(format_wstring) {
      ...
      assert::are_equal(L"string", string::format("{}", a), csf_);
    }
    
    void test_method_(format_without_register_any_stringer) {
      std::any a = foo {};
      assert::are_equal("(unregistered)", string::format("{}", a), csf_);
    }
