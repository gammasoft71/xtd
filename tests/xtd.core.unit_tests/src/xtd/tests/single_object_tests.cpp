...
namespace xtd::tests {
  class test_class_(single_object_tests) {
    ...
    void test_method_(max_value) {
      assert::are_equal(std::numeric_limits<float>::max(), single_object::max_value, csf_);
    }
    
    void test_method_(min_value) {
      assert::are_equal(std::numeric_limits<float>::lowest(), single_object::min_value, csf_);
    }
    ...
    void test_method_(negative_infinity) {
      assert::are_equal(-std::numeric_limits<float>::infinity(), single_object::negative_infinity, csf_);
    }
    
    void test_method_(positive_infinity) {
      assert::are_equal(std::numeric_limits<float>::infinity(), single_object::positive_infinity, csf_);
    }
    ...
    void test_method_(to_string) {
      ...
      assert::are_equal("2.328306E-10", single_object(0.00000000023283064365386962890625f).to_string(), csf_);
      assert::are_equal("1.2345E-35", single_object(1.2345E-35f).to_string(), csf_);
      assert::are_equal("1.234567", single_object(1.2345671f).to_string(), csf_);
      assert::are_equal("1.234568E+35", single_object(1.23456789E+35f).to_string(), csf_);
      assert::are_equal("1.401298E-45", single_object(single_object::epsilon).to_string(), csf_);
      assert::are_equal("-3.402823E+38", single_object(single_object::min_value).to_string(), csf_);
      assert::are_equal("3.402823E+38", single_object(single_object::max_value).to_string(), csf_);
