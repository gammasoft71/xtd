...
namespace xtd::net::tests {
  class test_class_(ip_address_tests) {
    ...
    auto test_method_(get_scope_id) {
      assert::throws<socket_exception>([] {[[maybe_unused]] auto _ = ip_address(xtd::array<xtd::byte> {172, 16, 10, 30}).scope_id();});
    ...
    auto test_method_(parse_with_bad_string) {
      assert::throws<format_exception>([] {[[maybe_unused]] auto _ = ip_address::parse("172.16.12.24.1");});
    }
  };
}
