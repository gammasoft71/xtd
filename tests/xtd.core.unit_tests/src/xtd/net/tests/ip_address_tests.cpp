...
namespace xtd::net::tests {
  ...
  public:
    ...
    void test_method_(broadcast) {
      assert::are_equal("255.255.255.255", ip_address::broadcast.to_string(), csf_);
    }
    ...
    void test_method_(loopback) {
      assert::are_equal("127.0.0.1", ip_address::loopback.to_string(), csf_);
    }
    
    void test_method_(none) {
      assert::are_equal("255.255.255.255", ip_address::none.to_string(), csf_);
    }
    
    void test_method_(default_constructor) {
      assert::are_equal("255.255.255.255", ip_address().to_string(), csf_);
    }
    
    void test_method_(constructor_with_uint32_t) {
      assert::are_equal("0.0.0.0", ip_address(0x00000000).to_string(), csf_);
      assert::are_equal("255.255.255.255", ip_address(0xFFFFFFFF).to_string(), csf_);
      assert::are_equal("172.0.10.35", ip_address(0x230A00AC).to_string(), csf_);
    }
    ...
    void test_method_(constructors_wtith_bytes_array_ip_v4) {
      assert::are_equal("172.16.10.30", ip_address(vector<xtd::byte> {172, 16, 10, 30}).to_string(), csf_);
    }
    
    void test_method_(constructors_wtih_bytes_array_ip_v6) {
      assert::are_equal("2001:0:5ef5:73b8:2c2c:3028:2a4e:b283", ip_address({0x20, 0x01, 0x00, 0x00, 0x5e, 0xf5, 0x73, 0xb8, 0x2c, 0x2c, 0x30, 0x28, 0x2a, 0x4e, 0xb2, 0x83}).to_string(), csf_);
      assert::are_equal("2001:0:5ef5:73b8:2c2c:3028:2a4e:b283%15", ip_address(vector<xtd::byte> {0x20, 0x01, 0x00, 0x00, 0x5e, 0xf5, 0x73, 0xb8, 0x2c, 0x2c, 0x30, 0x28, 0x2a, 0x4e, 0xb2, 0x83}, 15).to_string(), csf_);
    }
    ...
    void test_method_(constructors_wtih_quad_part) {
      ...
      assert::are_equal("10.11.12.35", ip_address(10, 11, 12, 35).to_string(), csf_);
      assert::are_equal("255.255.255.255", ip_address(255, 255, 255, 255).to_string(), csf_);
    }
    
    void test_method_(address_family) {
      assert::are_equal(address_family::inter_network, ip_address(vector<xtd::byte> {172, 16, 10, 30}).address_family(), csf_);
      assert::are_equal(address_family::inter_network_v6, ip_address(vector<xtd::byte> {0x20, 0x01, 0x00, 0x00, 0x5e, 0xf5, 0x73, 0xb8, 0x2c, 0x2c, 0x30, 0x28, 0x2a, 0x4e, 0xb2, 0x83}).address_family(), csf_);
    }
    
    void test_method_(get_scope_id) {
      assert::throws<socket_exception>([] {ip_address(vector<xtd::byte> {172, 16, 10, 30}).scope_id();}, csf_);
      assert::are_equal(0U, ip_address(vector<xtd::byte> {0x20, 0x01, 0x00, 0x00, 0x5e, 0xf5, 0x73, 0xb8, 0x2c, 0x2c, 0x30, 0x28, 0x2a, 0x4e, 0xb2, 0x83}).scope_id(), csf_);
      assert::are_equal(15U, ip_address(vector<xtd::byte> {0x20, 0x01, 0x00, 0x00, 0x5e, 0xf5, 0x73, 0xb8, 0x2c, 0x2c, 0x30, 0x28, 0x2a, 0x4e, 0xb2, 0x83}, 15).scope_id(), csf_);
    }
    ...
    void test_method_(host_to_network_order) {
      if (bit_converter::is_little_endian) {
        ...
        assert::are_equal(0x1234U, ip_address::host_to_network_order(static_cast<uint16_t>(0x1234)), csf_);
    ...
    void test_method_(is_loopback) {
      assert::is_false(ip_address::is_loopback(ip_address(172, 16, 12, 24)), csf_);
      assert::is_false(ip_address::is_loopback(ip_address(0, 0, 0, 0)), csf_);
      assert::is_true(ip_address::is_loopback(ip_address(127, 0, 0, 1)), csf_);
      assert::is_true(ip_address::is_loopback(ip_address::loopback), csf_);
      assert::is_true(ip_address::is_loopback(ip_address::ip_v6_loopback), csf_);
    }
    
    void test_method_(network_to_host_order) {
      if (bit_converter::is_little_endian) {
        assert::are_equal(0x3412U, ip_address::network_to_host_order(static_cast<uint16_t>(0x1234)), csf_);
        ...
        assert::are_equal(0x1234U, ip_address::network_to_host_order(static_cast<uint16_t>(0x1234)), csf_);
    ...
    void test_method_(parse_ip_v4) {
      assert::are_equal("172.16.12.24", ip_address::parse("172.16.12.24").to_string(), csf_);
    }
    
    void test_method_(parse_ip_v6) {
      assert::are_equal("2001:0:5ef5:73b8:2c2c:3028:2a4e:b283", ip_address::parse("2001:0:5ef5:73b8:2c2c:3028:2a4e:b283").to_string(), csf_);
