...
namespace xtd::net::sockets::tests {
  class test_class_(socket_tests) {
    ...
    auto test_method_(default_constructor) {
      ...
      assert::throws<object_closed_exception>([&] {[[maybe_unused]] auto _ = s.available();});
      assert::throws<object_closed_exception>([&] {[[maybe_unused]] auto _ = s.blocking();});
      assert::is_false(s.connected());
      assert::throws<object_closed_exception>([&] {[[maybe_unused]] auto _ = s.dont_fragment();});
      assert::throws<object_closed_exception>([&] {[[maybe_unused]] auto _ = s.dual_mode();});
      assert::throws<object_closed_exception>([&] {[[maybe_unused]] auto _ = s.enable_broadcast();});
      assert::throws<object_closed_exception>([&] {[[maybe_unused]] auto _ = s.exclusive_address_use();});
      assert::is_zero(s.handle());
      assert::is_false(s.is_bound());
      assert::throws<object_closed_exception>([&] {[[maybe_unused]] auto _ = s.linger_state();});
      assert::throws<object_closed_exception>([&] {[[maybe_unused]] auto _ = s.local_end_point();});
      assert::throws<object_closed_exception>([&] {[[maybe_unused]] auto _ = s.multicast_loopback();});
      assert::throws<object_closed_exception>([&] {[[maybe_unused]] auto _ = s.no_delay();});
      assert::are_equal(protocol_type::unspecified, s.protocol_type());
      assert::throws<object_closed_exception>([&] {[[maybe_unused]] auto _ = s.receive_buffer_size();});
      assert::throws<object_closed_exception>([&] {[[maybe_unused]] auto _ = s.receive_timeout();});
      assert::throws<object_closed_exception>([&] {[[maybe_unused]] auto _ = s.remote_end_point();});
      assert::throws<object_closed_exception>([&] {[[maybe_unused]] auto _ = s.send_buffer_size();});
      assert::throws<object_closed_exception>([&] {[[maybe_unused]] auto _ = s.send_timeout();});
      assert::are_equal(socket_type::unknown, s.socket_type());
      assert::throws<object_closed_exception>([&] {[[maybe_unused]] auto _ = s.ttl();});
    }
    ...
    auto test_method_(constructor_with_socket_type_stream_and_protocol_type_tcp) {
      ...
      assert::is_false(s.connected());
      assert::throws<not_supported_exception>([&] {[[maybe_unused]] auto _ = s.dont_fragment();});
      assert::is_true(s.dual_mode());
      assert::throws<socket_exception>([&] {[[maybe_unused]] auto _ = s.enable_broadcast();});
      ...
      assert::throws<socket_exception>([&] {[[maybe_unused]] auto _ = s.multicast_loopback();});
    ...
    auto test_method_(constructor_with_address_family_internet_network_socket_type_stream_and_protocol_type_tcp) {
      ...
      assert::throws<not_supported_exception>([&] {[[maybe_unused]] auto _ = s.dual_mode();});
      assert::throws<socket_exception>([&] {[[maybe_unused]] auto _ = s.enable_broadcast();});
      assert::is_false(s.exclusive_address_use());
      assert::is_not_zero(s.handle());
      assert::is_false(s.is_bound());
    ...
    auto test_method_(constructor_with_address_family_internet_network_socket_type_dgram_and_protocol_type_udp) {
      ...
      assert::is_not_zero(s.handle());
      assert::is_false(s.is_bound());
      assert::throws<socket_exception>([&] {[[maybe_unused]] auto _ = s.linger_state();});
      assert::is_null(s.local_end_point());
      assert::is_true(s.multicast_loopback());
      assert::throws<socket_exception>([&] {[[maybe_unused]] auto _ = s.no_delay();});
