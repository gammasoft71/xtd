...
namespace xtd::threading::tests {
  class test_class_(countdown_event_tests) {
    ...
    auto test_method_(close) {
      auto ce = countdown_event {0};
      ce.close();
      assert::throws<object_closed_exception>([&] {[[maybe_unused]] auto _ = ce.current_count();});
      assert::throws<object_closed_exception>([&] {[[maybe_unused]] auto _ = ce.initial_count();});
      assert::throws<object_closed_exception>([&] {[[maybe_unused]] auto _ = ce.is_set();});
    ...
    auto test_method_(signal_with_negative_param) {
      auto ce = countdown_event {10};
      assert::throws<argument_out_of_range_exception>([&] {ce.signal(static_cast<usize>(-1));});
    }
