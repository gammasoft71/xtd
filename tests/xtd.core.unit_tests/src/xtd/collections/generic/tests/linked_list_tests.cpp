...
namespace xtd::collections::generic::tests {
  class test_class_(linked_list_tests) {
    ...
    auto test_method_(add_after_node_stale) {
      ...
      assert::throws<invalid_operation_exception>([&] {[[maybe_unused]] auto _ = first_node.value();});
      assert::throws<invalid_operation_exception>([&] {[[maybe_unused]] auto _ = node.value();});
      assert::throws<invalid_operation_exception>([&] {[[maybe_unused]] auto _ = last_node.value();});
    }
    ...
    auto test_method_(iterator_on_empty_list) {
      ...
      assert::throws<invalid_operation_exception>([&] {[[maybe_unused]] auto _ = *iterator;});
    }
