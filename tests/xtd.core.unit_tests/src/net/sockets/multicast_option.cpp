...
namespace unit_tests {
  ...
  public:
    void test_method_(default_constructor) {
      multicast_option mo;
      assert::are_equal(ip_address::none, mo.group(), line_info_);
      assert::is_zero(mo.interface_index(), line_info_);
      assert::are_equal(ip_address::none, mo.local_adress(), line_info_);
    }
    ...
    void test_method_(constructor_with_group_and_local_address) {
      ...
      assert::are_equal("172.16.12.24", mo.group().to_string(), line_info_);
      assert::is_zero(mo.interface_index(), line_info_);
      assert::are_equal("172.16.12.1", mo.local_adress().to_string(), line_info_);
    }
    ...
    void test_method_(constructor_with_group) {
      ...
      assert::are_equal("172.16.12.24", mo.group().to_string(), line_info_);
      assert::is_zero(mo.interface_index(), line_info_);
      assert::are_equal(ip_address::any, mo.local_adress(), line_info_);
   }

    void test_method_(constructor_with_group_and_interface_index) {
      multicast_option mo(ip_address(172, 16, 12, 24), 427);
      assert::are_equal("172.16.12.24", mo.group().to_string(), line_info_);
      assert::are_equal(427U, mo.interface_index(), line_info_);
      assert::are_equal(ip_address::none, mo.local_adress(), line_info_);
    }
    
    void test_method_(constructor_with_group_and_interface_index_greather_than_0x00FFFFFF) {
      assert::throws<argument_out_of_range_exception>([]{multicast_option(ip_address(172, 16, 12, 24), 0x1000000);}, line_info_);
    }
    ...
    void test_method_(set_interface_index_greather_than_0x00FFFFFF) {
      multicast_option mo;
      assert::throws<argument_out_of_range_exception>([&]{mo.interface_index(0x1000000);}, line_info_);
    }
