...
class xtd_data_generator {
  static void Main() {
    ...
    Console.Write("  * Generate Regions...         ");
    Console.WriteLine(GenerateRegions(Path.Combine(new[] {xtdDataPath, "regions.bin"}), version) ? "[SUCCEED]" : "[FAILED ]");
  ...
  static bool GenerateRegions(string file_path, int version) {
    try {
      using (var bw = new System.IO.BinaryWriter(File.Open(file_path, FileMode.Create), Encoding.UTF8)) {
        bw.Write(new byte[] { (byte)'X', (byte)'T', (byte)'D', 0, (byte)'N', (byte)'U', (byte)'M', (byte)'F' });
        bw.Write(version);
