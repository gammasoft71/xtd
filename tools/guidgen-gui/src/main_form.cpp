  ...
  form_main() {
    ...
    generate_button_.click += [&] {
      ...
      thread generate([&]{
        ...
        for (int count = 0; count < count_numeric_up_down_.value(); count++) {
          begin_invoke([&] {
...
private:
  label count_label_;
  numeric_up_down count_numeric_up_down_;
  label format_label_;
  choice format_choice_;
  button generate_button_;
  text_box result_text_box_;
};
