...
void list_box::begin_update(intmax_t control) {
  if (control == 0) return;
}

void list_box::delete_item(intmax_t control, size_t index) {
  if (control == 0) return;
  static_cast<Fl_Browser*>(reinterpret_cast<control_handler*>(control)->control())->remove(index);
}

void list_box::end_update(intmax_t control) {
  if (control == 0) return;
}

void list_box::insert_item(intmax_t control, size_t index, const ustring& value) {
  if (control == 0) return;
