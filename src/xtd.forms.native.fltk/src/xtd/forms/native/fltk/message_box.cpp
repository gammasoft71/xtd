...
namespace {
  int_least32_t result_for_ok_cancel(int_least32_t result) {
    switch (result) {
      case 1: return IDOK;
  ...
  int_least32_t result_for_abort_retry_ignore(int_least32_t result) {
    switch (result) {
  ...
  int_least32_t result_for_yes_no_cancel(int_least32_t result) {
    switch (result) {
      case 0: return IDYES;
      case 1: return IDNO;
      case 2: return IDCANCEL;
  ...
  int_least32_t result_for_yes_no(int_least32_t result) {
    switch (result) {
      case 1: return IDYES;
      case 2: return IDNO;
    }
  ...
  int_least32_t result_for_retry_cancel(int_least32_t result) {
    switch (result) {
      case 1: return IDRETRY;
      case 2: return IDCANCEL;
    }
  ...
  int_least32_t convert_to_dialog_result(int_least32_t result, uint_least32_t style) {
  ...
  vector<string> convert_to_buttons(uint_least32_t style) {
  ...
  void convert_to_colored_icon(uint_least32_t style) {
...
int_least32_t message_box::show(intmax_t control, const ustring& text, const ustring& caption, uint_least32_t style, bool display_help_button) {
  ...
  int_least32_t result = fl_choice("%s", buttons.size() >= 1 ? buttons[0].c_str() : nullptr, buttons.size() >= 2 ? buttons[1].c_str() : nullptr, buttons.size() >= 3 ? buttons[2].c_str() : nullptr, text.c_str());
