...
void checked_list_box::begin_update(intmax_t control) {
  if (control == 0) return;
}

void checked_list_box::check_state(intmax_t control, size_t index, int check_state) {
  if (control == 0) return;
  static_cast<Fl_Check_Browser*>(reinterpret_cast<control_handler*>(control)->control())->checked(index, check_state != 0);
}

int checked_list_box::check_state(intmax_t control, size_t index) {
  if (control == 0) return false;
  return static_cast<Fl_Check_Browser*>(reinterpret_cast<control_handler*>(control)->control())->checked(index);
}

void checked_list_box::delete_item(intmax_t control, size_t index) {
  if (control == 0) return;
  static_cast<Fl_Check_Browser*>(reinterpret_cast<control_handler*>(control)->control())->remove(index);
}

void checked_list_box::end_update(intmax_t control) {
  if (control == 0) return;
}

void checked_list_box::insert_item(intmax_t control, size_t index, const ustring& value, bool checked) {
  if (control == 0) return;
...
size_t checked_list_box::selected_index(intmax_t control) {
  if (control == 0) return -1;
  return -1;
}

void checked_list_box::selected_index(intmax_t control, size_t index) {
  if (control == 0) return;
}

vector<size_t> checked_list_box::selected_indices(intmax_t control) {
  if (control == 0) return {};
  return {};
