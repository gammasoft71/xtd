  ...
  image blur_horizontal(const image& source_image, int32 radius) {
    ...
    for (auto y = 0; y < height; ++y) {
      ...
      for (auto kernel_x = -radius; kernel_x <= radius; ++kernel_x) {
        auto pixel = kernel_x < 0 ? y * width : kernel_x + y * width;
        sum_r += source_rgb[pixel].r;
        sum_g += source_rgb[pixel].g;
        sum_b += source_rgb[pixel].b;
        sum_a += source_alpha[pixel].a;
      }
      
      result_rgb[y * width].r = static_cast<byte>(sum_r / blur_area);
      result_rgb[y * width].g = static_cast<byte>(sum_g / blur_area);
      result_rgb[y * width].b = static_cast<byte>(sum_b / blur_area);
      result_alpha[y * width].a = static_cast<byte>(sum_a / blur_area);
      
      for (auto x = 1; x < width; ++x) {
        auto pixel = x - radius - 1 < 0 ? y * width : (x - radius - 1) + y * width;
        sum_r -= source_rgb[pixel].r;
        sum_g -= source_rgb[pixel].g;
        sum_b -= source_rgb[pixel].b;
        sum_a -= source_alpha[pixel].a;
        
        pixel = x + radius > width - 1 ? width - 1 + y * width :  x + radius + y * width;
        sum_r += source_rgb[pixel].r;
        sum_g += source_rgb[pixel].g;
        sum_b += source_rgb[pixel].b;
        sum_a += source_alpha[pixel].a;
        
        result_rgb[x + y * width].r = static_cast<byte>(sum_r / blur_area);
        result_rgb[x + y * width].g = static_cast<byte>(sum_g / blur_area);
        result_rgb[x + y * width].b = static_cast<byte>(sum_b / blur_area);
        result_alpha[x + y * width].a = static_cast<byte>(sum_a / blur_area);
      }
    }
...
image blur_vertical(const image& source_image, int32 radius) {
  ...
  for (auto x = 0; x < width; ++x) {
    ...
    for (auto kernel_y = -radius; kernel_y <= radius; ++kernel_y) {
      auto pixel = kernel_y < 0 ? x : x + kernel_y * width;
      sum_a += source_alpha[pixel].a;
      sum_r += source_rgb[pixel].r;
      sum_g += source_rgb[pixel].g;
      sum_b += source_rgb[pixel].b;
    ...
    result_alpha[x].a = static_cast<byte>(sum_a / blur_area);
    result_rgb[x].r = static_cast<byte>(sum_r / blur_area);
    result_rgb[x].g = static_cast<byte>(sum_g / blur_area);
    result_rgb[x].b = static_cast<byte>(sum_b / blur_area);
    
    for (auto y = 1; y < height; ++y) {
      auto pixel = y - radius - 1 < 0 ? x : x + (y - radius - 1) * width;
      sum_a -= source_alpha[pixel].a;
      sum_r -= source_rgb[pixel].r;
      sum_g -= source_rgb[pixel].g;
      sum_b -= source_rgb[pixel].b;
      
      pixel = y + radius > height - 1 ? x + (height - 1) * width : x + (radius + y) * width;
      sum_a += source_alpha[pixel].a;
      sum_r += source_rgb[pixel].r;
      sum_g += source_rgb[pixel].g;
      sum_b += source_rgb[pixel].b;
