...
void sepia_effect::apply(xtd::drawing::image& image) const {
  ...
  for (auto y = 0; y < image.height(); ++y)
    for (auto x = 0; x < image.width(); ++x) {
      ...
      rgb[pixel].r = helpers::rgb::alpha_blend(rgb[pixel].r, static_cast<byte>(std::clamp(0.393 * value.r + 0.769 * value.g + 0.189 * value.b, .0, 255.0)), percent);
      rgb[pixel].g = helpers::rgb::alpha_blend(value.g, static_cast<byte>(std::clamp(0.349 * value.r + 0.686 * value.g + 0.168 * value.b, .0, 255.0)), percent);
      rgb[pixel].b = helpers::rgb::alpha_blend(value.b, static_cast<byte>(std::clamp(0.272 * value.r + 0.534 * value.g + 0.131 * value.b, .0, 255.0)), percent);
    }
}
