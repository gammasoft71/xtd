...
font_family::font_family(const string& name) : data_(xtd::new_sptr<data>()) {
  data_->name_ = name;
  if (data_->name_.empty()) throw xtd::argument_exception("name is an empty string"_t);
  data_->handle_ = native::font_family::create(name);
  if (data_->handle_ == 0) throw xtd::argument_exception("name specifies a font that is not installed on the computer running the application."_t);
}
...
font_family::font_family(text::generic_font_families generic_font_families) : data_(xtd::new_sptr<data>()) {
  switch (generic_font_families) {
    ...
    default: throw xtd::argument_exception("name specifies a font that is not installed on the computer running the application."_t);
  }
}
