...
float color::get_brightness() const noexcept {
  ...
  return static_cast<float>(max(max(r(), g()), b())) / 255.0f;
}
...
float color::get_hue() const noexcept {
  ...
  auto max = static_cast<float>(std::max(std::max(rc, gc), bc));
  auto min = static_cast<float>(std::min(std::min(rc, gc), bc));
...
float color::get_lightness() const noexcept {
  return (static_cast<float>(max(max(r(), g()), b())) + static_cast<float>(min(min(r(), g()), b()))) / 2.0f / 255.0f;
}

float color::get_saturation() const noexcept {
  auto max = static_cast<float>(std::max(std::max(r(), g()), b())) / 255.0f;
  auto min = static_cast<float>(std::min(std::min(r(), g()), b())) / 255.0f;
