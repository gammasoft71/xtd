...
time_zone_info::transition_time time_zone_info::transition_time::create_fixed_date_rule(date_time time_of_day, uint32 month, uint32 day) {
  if (time_of_day.year() != 1 || time_of_day.month() != 1 || time_of_day.day() != 1 || time_of_day.kind() != date_time_kind::unspecified) xtd::helpers::throw_helper::throws(xtd::helpers::exception_case::argument);
  if (month < 1 || month > 12 || day < 1 || day > 31) xtd::helpers::throw_helper::throws(xtd::helpers::exception_case::argument_out_of_range);
...
time_zone_info::transition_time time_zone_info::transition_time::create_floating_date_rule(date_time time_of_day, uint32 month, uint32 week, xtd::day_of_week day_of_week) {
  if (time_of_day.year() != 1 || time_of_day.month() != 1 || time_of_day.day() != 1 || time_of_day.kind() != date_time_kind::unspecified) xtd::helpers::throw_helper::throws(xtd::helpers::exception_case::argument);
  if (month < 1 || month > 12 || week < 1 || week > 5) xtd::helpers::throw_helper::throws(xtd::helpers::exception_case::argument_out_of_range);
  auto result = transition_time {};
