...
auto yaml::as_string(const xtd::string& key) const -> string_type {
  return xtd::as<string_type>(nodes_[key]);
}
...
auto yaml::is_integer(const xtd::string& key) const noexcept -> bool {
  return contains_key(key) && (xtd::is<integer_type>(nodes_[key]) || xtd::is<uint64>(nodes_[key]));
}
...
auto yaml::is_string(const xtd::string& key) const noexcept -> bool {
  return contains_key(key) && xtd::is<string_type>(nodes_[key]);
}
