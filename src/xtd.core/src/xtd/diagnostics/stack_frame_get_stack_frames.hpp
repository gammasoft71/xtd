...
inline auto xtd::diagnostics::stack_frame::get_stack_frames(const xtd::string& str, xtd::usize skip_frames, bool need_file_info) noexcept {
  auto call_stack = xtd::native::stack_trace::get_frames(skip_frames, need_file_info);
  auto skip_frames_before_str = xtd::usize {0};
  if (!xtd::string::is_empty(str)) {
    skip_frames_before_str = call_stack.size();
    for (auto index = xtd::usize {0}; index < call_stack.size(); ++index) {
