...
int32 process::base_priority() const {
  if (!data_->handle_.has_value()) throw_helper::throws(exception_case::invalid_operation);
...
int32 process::exit_code() const {
  if (!data_->handle_.has_value() || !has_exited()) throw_helper::throws(exception_case::invalid_operation);
...
bool process::has_exited() const {
  if (!data_->handle_.has_value() || !data_->exit_code_.has_value()) throw_helper::throws(exception_case::invalid_operation);
...
process& process::priority_class(process_priority_class value) {
  ...
  if (native::process::priority_class(data_->handle_.value(), it->second) == false) throw_helper::throws(exception_case::invalid_operation);
  return *this;
}
...
bool process::start() {
  ...
  data_->thread_ = std::thread {[](class process process, bool & allow_to_continue) {
    try {
      ...
      if (process.data_->handle_ == 0) throw_helper::throws(exception_case::invalid_operation, "The system cannot find the file specified"_t);
      ...
      if (!process.data_->exit_code_.has_value() || process.data_->exit_code_ == -1 || process.data_->exit_code_ == 0x00ffffff) throw_helper::throws(exception_case::invalid_operation, "The system cannot find the file specified");
      process.on_exited();
    } catch (...) {
