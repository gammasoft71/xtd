...
stack_trace::stack_trace(const std::exception& exception) : stack_trace(ustring::full_class_name(exception), METHODS_TO_SKIP, false) {
}

stack_trace::stack_trace(const std::exception& exception, bool need_file_info) : stack_trace(ustring::full_class_name(exception), METHODS_TO_SKIP, need_file_info) {
}

stack_trace::stack_trace(const std::exception& exception, size_t skip_frames) : stack_trace(ustring::full_class_name(exception), skip_frames, false) {
}
...
const xtd::diagnostics::stack_frame& stack_trace::get_frame(size_t index) noexcept {
  if (frames_.size() == 0) return xtd::diagnostics::stack_frame::empty();
