...
string stack_trace::to_string(size_t skip_frames, const stack_frame& stack_frame) const noexcept {
  ...
  for (auto index = skip_frames; index < data_->frames.size(); ++index) {
    ...
    if (index == skip_frames && data_->need_file_info && stack_frame != stack_frame::empty()) str += string::format(" in {}:line {}", stack_frame.get_file_name(), stack_frame.get_file_line_number());
