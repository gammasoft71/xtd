...
trace_listener_collection::const_reference trace_listener_collection::operator[](const std::string& name) const {
  for(auto& item : *this)
    if(item->name() == name) return item;
  return empty_;
}

trace_listener_collection::reference trace_listener_collection::operator[](const std::string& name) {
  for(auto& item : *this)
    if(item->name() == name) return item;
  return empty_;
}
