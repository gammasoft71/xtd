...
std::vector<stack_frame> stack_frame::get_stack_frames(const std::string& str, size_t skip_frames, bool need_file_info) {
  ...
  for (size_t index = skip_frames_before_str + skip_frames; index < call_stack.stack.size() - 1; ++index)
    stack_frames.emplace_back(stack_frame(need_file_info ? call_stack.stack[index].file : "", need_file_info ? call_stack.stack[index].line : 0, call_stack.stack[index].function, need_file_info ? call_stack.stack[index].column : 0, call_stack.stack[index].offset));
  return stack_frames;
}
