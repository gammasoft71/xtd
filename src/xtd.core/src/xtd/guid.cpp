...
guid::guid(const array<byte>& data) : data_(data) {
  if (data.length() != data_.size()) throw_helper::throws(exception_case::argument, string::format("Vector size must be {}", data_.size()).chars().c_str());
}
...
guid::guid(uint32 a, uint16 b, uint16 c, const array<byte>& d) {
  if (d.length() != data_.size() - 8) throw_helper::throws(exception_case::argument, string::format("Vector size must be {}", data_.size() - 8).chars().c_str());
