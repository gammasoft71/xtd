/// @file
/// @brief Contains xtd::number concept alias documentation.
/// @copyright Copyright (c) 2026 Gammasoft. All rights reserved.
#pragma once
#error "Do not include this file: Internal use only. Include <xtd/number> or <xtd/number.hpp> instead."

/// @brief The xtd namespace contains all fundamental classes to access Hardware, Os, System, and more.
namespace xtd {
  /// @brief Represents the number concept alias.
  /// @par Definition
  /// ```cpp
  /// template<typename value_t>
  /// concept xtd::number = xtd::numeric<value_t>;
  /// ```
  /// @par Header
  /// ```cpp
  /// #include <xtd/number>
  ...
  /// @par Examples
  /// The following code shows how to use xtd::number concept
  /// ```cpp
  /// #include <xtd/xtd>
  ///
  /// auto print_is_number(auto value) noexcept {
  ///   println("{} ({}) {} number", value, type_of(value), number<decltype(value)> ? "is" : "is not");
  /// }
  ///
  /// auto main() -> int {
  ...
  ///   print_is_number('a');
  ///   print_is_number(true);
  ///   print_is_number("string"_s);
  ///   print_is_number("string");
  ///   print_is_number(1_h + 12_min + 24_s + 48_ms);
  ///   print_is_number(day_of_week::wednesday);
  /// }
  ///
