    ...
    namespace views {
      ...
      class where_view {
        ...
        struct with_predicate {
          ...
          auto operator()(iterable_t&& source) const {return xtd::linq::enumerable::as_enumerable(std::forward<iterable_t>(source)).where(predicate);}
          
          template <xtd::iterable iterable_t>
          friend auto operator |(iterable_t&& source, const with_predicate& self) {return self(std::forward<iterable_t>(source));}
        };
      ...
      public:
        ...
        template <xtd::iterable iterable_t, typename predicate_t>
        auto operator()(iterable_t&& source, const predicate_t& predicate) const {return xtd::linq::enumerable::from(std::forward<iterable_t>(source)).where(predicate);}
        /// @brief Filters a sequence of values based on a predicate.
        /// @tparam range_t The type of the elements of source.
        /// @param predicate A function to test each element for a condition.
        template<typename predicate_t>
        ...
        /// @endcode
        template <xtd::iterable iterable_t>
        friend auto operator |(iterable_t&& source, const where_view& view) {return view(std::forward<iterable_t>(source));}
        /// @}
      };
    }
  }
