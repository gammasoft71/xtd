...
struct __polymorphic_equator__<type_t, std::true_type> {
  bool operator()(const type_t& a, const type_t& b) const {
    if (dynamic_cast<const xtd::iequatable<type_t>*>(&a)) return dynamic_cast<const xtd::iequatable<type_t>&>(a).equals(b);
    if (dynamic_cast<const xtd::object*>(&a)) return dynamic_cast<const xtd::object&>(a).equals(dynamic_cast<const xtd::object&>(b));
