...
namespace xtd {
  ...
  namespace expressions {
    ...
    struct bitwise_xor_expression : expression {
      /// @name Public Fields
      ...
      constexpr bitwise_xor_expression(left_t left, right_t right) : left {std::move(left)}, right {std::move(right)} {}
      /// @}
      ...
      /// @cond
      friend inline auto operator <<(std::ostream& os, const bitwise_xor_expression& e) -> std::ostream& {return os << expression_stream {e.left, e.precedence} << " ^ " << expression_stream {e.right, e.precedence};}
      /// @endcond
