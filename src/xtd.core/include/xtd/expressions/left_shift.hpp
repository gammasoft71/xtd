/// @file
/// @brief Contains xtd::expressions::left_shift operator.
...
namespace xtd {
  ...
  namespace expressions {
    /// @brief The xtd::expressions::left_shift is the bitwise left expression.
    /// @par Namespace
    /// xtd::expressions
    /// @par Header
    /// ```cpp
    /// #include <xtd/expressions/left_shift>
    /// ```
    /// @par Library
    /// xtd.core
    /// @ingroup xtd_core
    /// @remarks The xtd::expressions::left_shift struct is used by xtd::expressions::expression::left_shift expression.
    template<typename left_t, typename right_t>
    struct left_shift : binary_expression {
      /// @name Public Fields
      ...
      /// @{
      /// @brief Initialize a new xtd::expressions::left_shift object.
      constexpr left_shift() = default;
      
      /// @brief Initialize a new xtd::expressions::left_shift object with specified left and right operands.
      /// @param left The left operand.
      /// @param right The right operand.
      constexpr left_shift(left_t left, right_t right) : left {std::move(left)}, right {std::move(right)} {}
      /// @}
      ...
      /// @cond
      friend inline auto operator <<(std::ostream& os, const left_shift& e) -> std::ostream& {return os << expression_stream {e.left, e.precedence} << " << " << expression_stream {e.right, e.precedence};}
      /// @endcond
    ...
    constexpr auto expression::left_shift(left_t left, right_t right) {
      ...
      return expressions::left_shift<std::decay_t<decltype(left_expression)>, std::decay_t<decltype(right_expression)>> {std::move(left_expression), std::move(right_expression)};
    }
    /// @endcond
