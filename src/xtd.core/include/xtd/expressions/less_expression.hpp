...
namespace xtd {
  ...
  namespace expressions {
    ...
    struct less_expression : expression_base {
      ...
      constexpr less_expression(auto left, auto right) : left {std::move(left)}, right {std::move(right)} {}
      /// @}
    ...
    constexpr auto operator <(left_t left, right_t right) {
      return less_expression<std::decay_t<decltype(as_expression(left))>, std::decay_t<decltype(as_expression(right))>> {as_expression(left), as_expression(right)};
    }
    /// @}
  }
}
