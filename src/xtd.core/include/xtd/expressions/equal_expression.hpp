...
namespace xtd {
  ...
  namespace expressions {
    ...
    struct equal_expression : expression_base {
      ...
      constexpr equal_expression(auto left, auto right) : left {std::move(left)}, right {std::move(right)} {}
      /// @}
    ...
    constexpr auto operator ==(left_t left, right_t right) {
      return equal_expression<std::decay_t<decltype(as_expression(left))>, std::decay_t<decltype(as_expression(right))>> {as_expression(left), as_expression(right)};
    }
    /// @}
  }
}
