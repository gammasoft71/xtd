...
namespace xtd {
  ...
  namespace expressions {
    ...
    /// @par Header
    /// ```cpp
    /// #include <xtd/expressions/bitwise_and_expression>
    /// ```
    /// @par Library
    /// xtd.core
    ...
    struct bitwise_and_expression : expression {
      ...
      constexpr bitwise_and_expression(left_t left, right_t right) : left {std::move(left)}, right {std::move(right)} {}
      /// @}
    ...
    constexpr auto operator &(left_t left, right_t right) {
      ...
      return bitwise_and_expression<std::decay_t<decltype(left_expression)>, std::decay_t<decltype(right_expression)>> {std::move(left_expression), std::move(right_expression)};
    }
    /// @}
  }
}
