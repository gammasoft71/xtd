...
namespace xtd {
  ...
  namespace expressions {
    ...
    struct unary_plus_expression : expression_base {
      ...
      constexpr unary_plus_expression(auto&& value) : value {std::forward<decltype(value)>(value)} {}
      /// @}
      ...
      /// @cond
      friend inline auto operator <<(std::ostream& os, const unary_plus_expression& e) -> std::ostream& {
        os << " +";
    ...
    constexpr auto operator +(value_t value) {
      return unary_plus_expression<decltype(as_expression(value))> {as_expression(value)};
    }
    /// @}
  }
}
