/// @file
/// @brief Contains xtd::expressions::remainder_expression opertor.
...
namespace xtd {
  ...
  namespace expressions {
    /// @brief The xtd::expressions::remainder_expression is the multiplication expression.
    /// @par Library
    /// xtd.core
    ...
    struct remainder_expression : expression_base {
      ...
      constexpr remainder_expression(auto&& left, auto&& right) : left {std::forward<decltype(left)>(left)}, right {std::forward<decltype(right)>(right)} {}
    ...
    constexpr auto operator %(left_t left, right_t right) {return remainder_expression<decltype(as_expression(left)), decltype(as_expression(right))> {as_expression(left), as_expression(right)};}
    /// @}
  }
}
