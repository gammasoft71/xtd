...
namespace xtd {
  ...
  namespace expressions {
    ...
    struct or_expression : expression_base {
      ...
      constexpr or_expression(auto&& left, auto&& right) : left {std::forward<decltype(left)>(left)}, right {std::forward<decltype(right)>(right)} {}
      /// @}
      ...
      /// @cond
      friend inline auto operator <<(std::ostream& os, const or_expression& e) -> std::ostream& {
    ...
    constexpr auto operator ||(left_t left, right_t right) {return or_expression<decltype(as_expression(left)), decltype(as_expression(right))> {as_expression(left), as_expression(right)};}
    /// @}
  }
}
