    ...
    namespace specialized {
      ...
      public:
        ...
        ordered_dictionary(const xtd::collections::generic::idictionary<key_t, value_t>& dictionary) {
          data_->items.capaciy(dictionary.count());
        ...
        void insert(xtd::size index, const key_t & key, const value_t value) {
          ...
          data_->items.insert(index, std::forward<base_value_type>({key, value}));
          ++data_->version;
        }
        
        bool remove(const key_t & key) noexcept override {
          auto index = get_index(key);
          if (index == xtd::npos) return false;
          remove_at(index);
          return true;
        }
        ...
        const value_t& operator [](const key_t & key) const override {
          auto index = get_index(key);
          if (index == xtd::npos) xtd::helpers::throw_helper::throws(xtd::helpers::exception_case::key_not_found);
      ...
      private:
        ...
        struct dictionary_data {
          ...
          dictionary_data(const base_type & items, size_type version) noexcept : items {size_type {}}, version {version} {
            for (const auto& item : items)
              this->items.add(item);
          }
          dictionary_data(base_type&& items, size_type version) noexcept : items {size_type {}}, version {version} {
            for (auto&& item : items)
              this->items.add(item);
          }
