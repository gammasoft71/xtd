...
namespace xtd {
  ...
  public:
    ...
    /// @remarks The method delegate is executed on the thread that created the object, instead of the thread on which xtd::isynchronize_invoke::begin_invoke was called.
    /// @remarks The delegate is called asynchronously, and this method returns immediately. You can call this method from any thread. If you need the return value from a process started with this method, call xtd::isynchronize_invoke::end_invoke to get the value.
    virtual auto begin_invoke(delegate<void(xtd::array<xtd::any_object>)> value, const xtd::array<xtd::any_object>& args) -> xtd::async_result = 0;
    ...
    /// @remarks Unlike xtd::isynchronize_invoke::begin_invoke, this method operates synchronously, that is, it waits until the process completes before returning. Exceptions raised during the call are propagated back to the caller.
    /// @remarks Use this method when calling a method from a different thread to marshal the call to the proper thread.
    virtual auto invoke(delegate<void(xtd::array<xtd::any_object>)> method, const xtd::array<xtd::any_object>& args) -> std::optional<object_ref> = 0;
    /// @brief Synchronously executes the delegate on the thread that created this object and marshals the call to the creating thread.
    /// @param method A xtd::delegate that contains a method to call, in the context of the thread for the control.
    ...
    /// @remarks Unlike xtd::isynchronize_invoke::begin_invoke, this method operates synchronously, that is, it waits until the process completes before returning. Exceptions raised during the call are propagated back to the caller.
    /// @remarks Use this method when calling a method from a different thread to marshal the call to the proper thread.
    virtual auto invoke(delegate<void(xtd::array<xtd::any_object>)> method, const xtd::any_object& arg) -> std::optional<object_ref> = 0;
    /// @}
    
    /// @cond
    template<typename delegate_t>
    auto begin_invoke(delegate_t value, const xtd::array<xtd::any_object>& args) -> xtd::async_result {return begin_invoke(delegate<void(xtd::array<xtd::any_object>)>(value), args);}
    
    template<typename delegate_t>
    auto begin_invoke(delegate_t value) -> xtd::async_result {return begin_invoke(delegate<void(xtd::array<xtd::any_object>)>(value), {});}
    
    template<typename delegate_t>
    auto invoke(delegate_t value, const xtd::array<xtd::any_object>& args) -> std::optional<object_ref> {return invoke(delegate<void(xtd::array<xtd::any_object>)>(value), args);}
    
    template<typename delegate_t, typename args_t>
    auto invoke(delegate_t value, args_t args) -> std::optional<object_ref> {return invoke(delegate<void(xtd::array<xtd::any_object>)>(value), xtd::any_object(args));}
    
    template<typename delegate_t>
    auto invoke(delegate_t value) -> std::optional<object_ref> {return invoke(delegate<void(xtd::array<xtd::any_object>)>(value), xtd::array<xtd::any_object> {});}
    /// @endcond
  };
}
