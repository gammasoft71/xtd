  ...
  namespace runtime {
    ...
    namespace exception_services {
      ...
      public:
        ...
        /// @{
        /// @brief Represents the optional exception value type.
        using exception_value = std::optional<std::reference_wrapper<const xtd::exception>>;
        /// @}
        ...
        exception_value source_exception() const noexcept {return source_;}
        /// @}
        
        /// @name Public Properties
        
        /// @{
        /// @brief Creates an xtd::runtime::exception_services::exception_dispatch_info object that represents the current exception at the current point in code.
        ...
        static exception_dispatch_info capture() {return exception_dispatch_info {true};}
        ...
        /// @remarks You can use the xtd::runtime::exception_services::exception_dispatch_info object that's returned by this method at another time and possibly on another thread to rethrow the specified exception, as if the exception had flowed from the point where it was captured to the point where it's rethrown.
        /// @remarks If the exception is active when it's captured, the current stack trace information that's contained in the exception is stored. If it's inactive, that is, if it has not been thrown, it doesn't have any stack trace information.
        static exception_dispatch_info capture(const xtd::exception& source) {return exception_dispatch_info {source};}
      ...
      private:
        exception_dispatch_info(bool) : exception_ptr_ {std::current_exception()} {}
        exception_dispatch_info(const xtd::exception& source) : exception_ptr_ {std::make_exception_ptr(source)}, source_ {source} {}
        
        std::exception_ptr exception_ptr_;
        exception_value source_;
      };
    }
  }
}
