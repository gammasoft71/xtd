/// @file
/// @brief Contains xtd::double_ suffixes.
/// @copyright Copyright (c) 2026 Gammasoft. All rights reserved.
#pragma once
#include <cstddef>
#include "../double.hpp"
#include "../usize.hpp"
#define __XTD_STD_INTERNAL__
#include "../internal/__xtd_std_version.hpp"
#undef __XTD_STD_INTERNAL__

/// @brief The xtd namespace contains all fundamental classes to access Hardware, Os, System, and more.
namespace xtd {
  /// @brief The xtd::literals namespace contains all xtd's litterals operators.
  inline namespace literals {
    /// @name Double suffixes
    
    /// @{
    /// @brief Used to convert specified value into byte.
    /// @par Namespace
    /// xtd
    /// @par Library
    /// xtd.core
    /// @ingroup xtd_core literals
    /// @par Examples
    /// ```cpp
    /// xtd::console::out << 1.6_d << xtd::environment::new_line;
    /// ```
    [[nodiscard]] auto operator""_d(long double n) -> double;
    
    /// @brief Used to convert specified value into byte.
    /// @par Namespace
    /// xtd
    /// @par Library
    /// xtd.core
    /// @ingroup xtd_core literals
    /// @par Examples
    /// ```cpp
    /// xtd::console::out << 54_d << xtd::environment::new_line;
    /// ```
    [[nodiscard]] auto operator""_d(unsigned long long n) -> double;
    
    /// @brief Used to convert specified value into byte.
    /// @par Namespace
    /// xtd
    /// @par Library
    /// xtd.core
    /// @ingroup xtd_core literals
    /// @par Examples
    /// ```cpp
    /// xtd::console::out << "125"_d << xtd::environment::new_line;
    /// ```
    [[nodiscard]] auto operator""_d(const char* s, xtd::usize n) -> double;
    
    /// @brief Used to convert specified value into byte.
    /// @par Namespace
    /// xtd
    /// @par Library
    /// xtd.core
    /// @ingroup xtd_core literals
    /// @par Examples
    /// ```cpp
    /// xtd::console::out << u8"125"_d << xtd::environment::new_line;
    /// ```
    [[nodiscard]] auto operator""_d(const char8_t* s, xtd::usize n) -> double;
    
    /// @brief Used to convert specified value into byte.
    /// @par Namespace
    /// xtd
    /// @par Library
    /// xtd.core
    /// @ingroup xtd_core literals
    /// @par Examples
    /// ```cpp
    /// xtd::console::out << u"125"_d << xtd::environment::new_line;
    /// ```
    [[nodiscard]] auto operator""_d(const char16_t* s, xtd::usize n) -> double;
    
    /// @brief Used to convert specified value into byte.
    /// @par Namespace
    /// xtd
    /// @par Library
    /// xtd.core
    /// @ingroup xtd_core literals
    /// @par Examples
    /// ```cpp
    /// xtd::console::out << U"125"_d << xtd::environment::new_line;
    /// ```
    [[nodiscard]] auto operator""_d(const char32_t* s, xtd::usize n) -> double;
    
    /// @brief Used to convert specified value into byte.
    /// @par Namespace
    /// xtd
    /// @par Library
    /// xtd.core
    /// @ingroup xtd_core literals
    /// @par Examples
    /// ```cpp
    /// xtd::console::out << L"125"_d << xtd::environment::new_line;
    /// ```
    [[nodiscard]] auto operator""_d(const wchar_t* s, xtd::usize n) -> double;
    /// @}
  }
}
