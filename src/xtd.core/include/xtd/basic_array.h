...
namespace xtd {  
  ...
  public:
    ...
    /// @cond
    basic_array(const basic_array& array) : data_ {xtd::new_ptr<array_data>()} { if (array.data_) *data_ = *array.data_;}
  ...
  private:
    ...
    basic_array() : data_ {xtd::new_ptr<array_data>()} {}
    basic_array(const array<size_type, 1>& lengths);
    
    basic_array(const_pointer array, size_type length) : data_ {xtd::new_ptr<array_data>()} {
      if (array == null) __throw_argument_null_exception(__FILE__, __LINE__, __func__);
      data_->items = base_type {array, array + length};
      data_->upper_bound[0] = data_->items.size() - 1;
    }
    
    basic_array(const xtd::collections::generic::ienumerable<type_t>& enumerable) : data_ {xtd::new_ptr<array_data>()} {
    ...
    basic_array(const std::vector<type_t>& array) : data_ {xtd::new_ptr<array_data>()} {
      data_->items = array;
      data_->upper_bound[0] = data_->items.size() - 1;
    }
    
    basic_array(std::vector<type_t>&& array) : data_ {xtd::new_ptr<array_data>()} {
      data_->items = std::move(array);
      data_->upper_bound[0] = data_->items.size() - 1;
    }
    
    basic_array(std::initializer_list<type_t> il) : data_ {xtd::new_ptr<array_data>()} {
      data_->items.assign(il.begin(), il.end());
      data_->upper_bound[0] = data_->items.size() - 1;
    }
    
    basic_array(std::initializer_list<std::initializer_list<type_t>> il) : data_ {xtd::new_ptr<array_data>()}  {
    ...
    basic_array(std::initializer_list<std::initializer_list<std::initializer_list<type_t>>> il) : data_ {xtd::new_ptr<array_data>()}  {
    ...
    basic_array(iterator_t first, iterator_t last) : data_ {xtd::new_ptr<array_data>()} {
