
/// @file
/// @brief Contains console special key constants.
/// @copyright Copyright (c) 2025 Gammasoft. All rights reserved.
#pragma once
/// @cond
#if !defined(__XTD_CORE_NATIVE_LIBRARY__)
#error "Do not include this file: Internal use only"
#endif
/// @endcond

#include <cstdint>

/// @brief The control modifier key plus the BREAK console key.
/// @par Library
/// xtd.core.native
/// @ingroup xtd_core_native console_special_key
/// @remarks Added specifically for xtd.
/// @warning Internal use only
constexpr std::int32_t CONSOLE_SPECIAL_KEY_CTRL_BREAK = 0;

/// @brief The control modifier key plus the C console key.
/// @par Library
/// xtd.core.native
/// @ingroup xtd_core_native console_special_key
/// @remarks Added specifically for xtd.
/// @warning Internal use only
constexpr std::int32_t CONSOLE_SPECIAL_KEY_CTRL_C = 1;

/// @brief The control modifier key plus the \\ console key.
/// @par Library
/// xtd.core.native
/// @ingroup xtd_core_native console_special_key
/// @remarks Added specifically for xtd.
/// @warning Internal use only
constexpr std::int32_t CONSOLE_SPECIAL_KEY_CTRL_BS = 2;

/// @brief The control modifier key plus the Z console key.
/// @par Library
/// xtd.core.native
/// @ingroup xtd_core_native console_special_key
/// @remarks Added specifically for xtd.
/// @warning Internal use only
constexpr std::int32_t CONSOLE_SPECIAL_KEY_CTRL_Z = 3;
