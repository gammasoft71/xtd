...
string file_system::get_full_path(const string& relative_path) {
  vector<string> directories = native::freebsd::strings::split(relative_path, {path::directory_separator_char()}, std::numeric_limits<size_t>::max(), true);
  string full_path;
  ...
  for (const string& item : directories) {
    if (item == ".." && native::freebsd::strings::last_index_of(full_path, path::directory_separator_char()) != full_path.npos)
      full_path = native::freebsd::strings::remove(full_path, native::freebsd::strings::last_index_of(full_path, path::directory_separator_char()));
    else if (item != ".")
  ...
  auto index = native::freebsd::strings::last_index_of(full_path, "/./");
  while (index != full_path.npos) {
    full_path = native::freebsd::strings::remove(full_path, index, 2);
    index = native::freebsd::strings::last_index_of(full_path, "/./");
  }
...
int32_t file_system::get_permissions(const std::string& path, int32_t& permissions) {
  ...
  struct stat s;
  int32_t ret_value = stat(path.c_str(), &s);
...
int32_t file_system::set_attributes(const std::string& path, int32_t attributes) {
  ...
  // The other attributes can be modified under freebsd.
  
  return chmod(path.c_str(), s.st_mode);
}

int32_t file_system::set_creation_time(const std::string& path, time_t creation_time) {
  // There is no creation time on freebsd so we update the last modification time instead...
  utimbuf times;
