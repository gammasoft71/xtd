...
void button::on_paint(paint_event_args& e) {
  if (flat_style_ == xtd::forms::flat_style::system)
    ...
    button_renderrer::draw_button(e.graphics(), e.clip_rectangle(), text(), font(), flags, image_, image_bounds, false, state_, !back_color_.has_value() && back_color() != xtd::forms::theme_color::current_theme_color().control() ? back_color() : back_color_, !fore_color_.has_value() && fore_color() != xtd::forms::theme_color::current_theme_color().control_text() ? fore_color() : fore_color_);
  }
}
