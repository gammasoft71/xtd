...
namespace xtd {
  ...
  namespace tunit {
    ...
    public:
      ...
      class_initialize_attribute(const std::string& name, test_class_t& test_class, void (*method)(), const xtd::diagnostics::stack_frame& stack_frame = xtd::diagnostics::stack_frame::current()) noexcept {test_class.add_class_initialize({name, method, stack_frame});}
      /// @}
    };
  }
}
...
#define class_initialize_(method_name) \
  ...
  struct __class_initialize_attribute : xtd::tunit::class_initialize_attribute { \
    template<class test_class> __class_initialize_attribute(test_class& test) : class_initialize_attribute(#method_name, test, &method_name) {__##method_name##_static();} \
