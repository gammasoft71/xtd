...
void ostream_event_listener::on_class_end(const class_event_args& e) const {
  if (settings::default_settings().gtest_compatibility()) {
    ...
    os_ << "[----------] ";
    console::reset_color();
    os_ << e.test_class().test_count() << " tests from " << e.test_class().name() << " (" << static_cast<int32>(e.test_class().elapsed_time().total_milliseconds()) << " ms total)" << std::endl << std::endl;
  }
...
void ostream_event_listener::on_unit_test_end(const tunit_event_args& e) const {
  ...
  if (settings::default_settings().gtest_compatibility()) {
    ...
    os_ << "[==========] ";
    console::reset_color();
    os_ << e.unit_test().test_count() << " tests from " << e.unit_test().test_cases_count() << " test suite" << (e.unit_test().test_cases_count() > 1 ?  "s" : "") <<  " ran. (" << static_cast<int32>(e.unit_test().elapsed_time().total_milliseconds()) << " ms total)" << std::endl;
    console::foreground_color(console_color::dark_green);
    os_ << "[  PASSED  ] ";
    console::reset_color();
