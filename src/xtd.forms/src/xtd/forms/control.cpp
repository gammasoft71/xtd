...
void control::on_handle_created(const event_args& e) {
  ...
  if ((!xtd::forms::theme_colors::current_theme().is_default() || data_->back_color.has_value() || back_color() != default_back_color()) && is_handle_created()) native::control::back_color(handle(), back_color());
