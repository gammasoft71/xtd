...
color_dialog& color_dialog::custom_colors(const colors& custom_colors)  {
  for (auto index = 0_z; index < custom_colors.length(); ++index)
    data_->custom_colors[index] = custom_colors[index];
  for (auto index = custom_colors.length(); index < data_->custom_colors.length(); ++index)
...
void color_dialog::run_sheet(intptr owner) {
  if (!owner) [[maybe_unused]] auto __ = run_dialog(owner);
