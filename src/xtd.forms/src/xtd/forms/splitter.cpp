...
control& splitter::dock(dock_style dock) {
  if (dock != dock_style::left && dock != dock_style::right && dock != dock_style::top && dock != dock_style::bottom) throw_helper::throws(exception_case::argument, "splitter control must be docked left, right, top, or bottom.");
  control::dock(dock);
...
void splitter::on_mouse_down(const mouse_event_args& e) {
  ...
  if (parent().has_value()) {
    for (auto index = 0_z; index < parent().value().get().controls().count(); ++index) {
      if (parent().value().get().controls()[index].get() == *this) {
        ...
        if (index < parent().value().get().controls().count() - 1) data_->next_control_cursor = (data_->next_control = &parent().value().get().controls()[index + 1].get())->cursor();
        break;
      }
    }
  }
