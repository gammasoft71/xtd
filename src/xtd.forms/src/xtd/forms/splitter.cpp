...
int splitter::min_size() const noexcept {
...
splitter& splitter::min_size(int min_size) {
...
void splitter::on_mouse_move(const mouse_event_args& e) {
  ...
  if (data_->mouse_down_location != -1 && data_->next_control) {
    if (data_->splitter_style == splitter_style::draw_line) {
      ...
      int delta_size = control::dock() == dock_style::left || control::dock() == dock_style::right ? (data_->next_control->width() + cursor::position().x()) : (data_->next_control->height() + cursor::position().y());
      int new_size = delta_size - data_->mouse_down_location;
...
void splitter::on_mouse_up(const mouse_event_args& e) {
  ...
  if (data_->mouse_down_location != -1 && parent().has_value() && data_->next_control) {
    int new_size = (control::dock() == dock_style::left || control::dock() == dock_style::right ? (data_->next_control->width() + cursor::position().x()) : (data_->next_control->height() + cursor::position().y())) - data_->mouse_down_location;
    if (new_size < data_->min_size) new_size = data_->min_size;
    if (control::dock() == dock_style::left || control::dock() == dock_style::right) data_->next_control->width(new_size);
    else data_->next_control->height(new_size);
