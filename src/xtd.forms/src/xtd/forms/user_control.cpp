...
void user_control::on_handle_created(const event_args& e) {
  ...
  // Workaround : on macOS with wxWidgets toolkit, retina display, dark mode enabled, border style is fixed 3d, the border is not show.
  parent().value().get().paint += [this](control& sender, paint_event_args& e) {
    if (parent().has_value() && environment::os_version().is_macos_platform() && native::toolkit::name() == "wxwidgets" && screen::from_handle(handle()).scale_factor() > 1. && application::dark_mode_enabled() && border_style_ == forms::border_style::fixed_3d)
