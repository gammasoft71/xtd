...
control& tab_page::parent(const control& parent) {
  if (!is<tab_control>(parent)) throw_helper::throws(exception_case::argument, string::format("tab_page cannot be added to a '{}'.  tab_page can only be added to tab_control", parent.get_type().full_name()).chars().c_str());
