...
const color_property color_property::empty {true};

color_property::color_property(const drawing::color& color) : color_({color}) {
}

const xtd::drawing::color& color_property::color() const noexcept {
  return color_;
}

void color_property::color(const xtd::drawing::color& value) noexcept {
  color_ = value;
}

bool color_property::from_css(const xtd::ustring& css_text, color_property& result) noexcept {
  return try_parse(css_text, result);
}

color_property color_property::from_css(const xtd::ustring& css_text, const color_property& default_value) noexcept {
  color_property result;
  if (from_css(css_text, result)) return result;
  return default_value;
}

solid_brush color_property::make_brush(const color_property& color) {
  return solid_brush(color.color());
}

color_property color_property::parse(const xtd::ustring& text) {
  color_property result;
...
xtd::ustring color_property::to_string() const noexcept {
  return ustring::format("[color={}]", color());
}

xtd::ustring color_property::to_css() const noexcept {
  return ustring::format("{}", color_to_string(color()));
}

bool color_property::try_parse(const xtd::ustring& text, color_property& result) noexcept {
...
xtd::ustring color_property::color_to_string(const drawing::color& color) {
...
ustring color_property::remove_key(const xtd::ustring& text) {
...
bool color_property::try_parse_hash_color(const xtd::ustring& text, color_property& result) {
  if (text.starts_with("#") && text.size() == 4U) {
    ...
    b += b * 16;
    result = color_property(color::from_argb(r, g, b));
    return true;
  }
...
bool color_property::try_parse_rgba_color(const xtd::ustring& text, color_property& result) {
...
bool color_property::try_parse_argb_color(const xtd::ustring& text, color_property& result) {
...
bool color_property::try_parse_hsv_color(const xtd::ustring& text, color_property& result) {
  ...
  result = color_property(color::from_hsb(h, s, v));
  return true;
}

bool color_property::try_parse_hsva_color(const xtd::ustring& text, color_property& result) {
  ...
  byte_t a = 0;
  if (xtd::try_parse<byte_t>(color_parts[3], a) == false) return false;
  result = color_property(color::from_argb(a, color::from_hsb(h, s, v)));
  return true;
}

bool color_property::try_parse_ahsv_color(const xtd::ustring& text, color_property& result) {
...
bool color_property::try_parse_hsl_color(const xtd::ustring& text, color_property& result) {
  ...
  result = color_property(color::from_hsl(h, s, l));
  return true;
}

bool color_property::try_parse_hsla_color(const xtd::ustring& text, color_property& result) {
  ...
  byte_t a = 0;
  if (xtd::try_parse<byte_t>(color_parts[3], a) == false) return false;
  result = color_property(color::from_argb(a, color::from_hsl(h, s, l)));
  return true;
}

bool color_property::try_parse_ahsl_color(const xtd::ustring& text, color_property& result) {
...
bool color_property::try_parse_named_color(const ustring& text, color_property& result) {
  if (text.trim().find(" ") != ustring::npos) return false;
  result = color_property(color::from_name(text));
  return result.color().is_known_color();
}

bool color_property::try_parse_system_color(const ustring& text, color_property& result) {
  ...
  result = color_property(it->second);
  return true;
}
