...
void image_data::angle(int32_t value) noexcept {
  angle_ = value;
}

const std::vector<xtd::drawing::color>& image_data::colors() const noexcept {
  return colors_;
}

void image_data::colors(const std::vector<xtd::drawing::color>& value) {
  colors_ = value;
  if (colors_.size() < 2U) throw argument_exception(csf_);
}

const xtd::ustring& image_data::url() const noexcept {
  return url_;
}

void image_data::url(const xtd::ustring& value) noexcept {
  url_ = value;
}
...
image_data image_data::parse(const xtd::ustring& text) {
  image_data result;
...
bool image_data::try_parse(const xtd::ustring& text, image_data& result) {
...
xtd::ustring image_data::color_to_string(const drawing::color& color) {
...
ustring image_data::remove_key(const xtd::ustring& text) {
