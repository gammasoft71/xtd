...
control& lcd_label::text(const xtd::string& value) {
  ...
  if (str.length() > data_->digits.count())
    for (auto index = data_->digits.count(); index < str.length(); ++index) {
      ...
      dynamic_cast<control*>(data_->digits[~1_i].get())->parent(*this);
      dynamic_cast<control*>(data_->digits[~1_i].get())->double_buffered(double_buffered());
      dynamic_cast<control*>(data_->digits[~1_i].get())->click += {*this, &lcd_label::on_digit_click};
      dynamic_cast<control*>(data_->digits[~1_i].get())->mouse_down += {*this, &lcd_label::on_digit_mouse_down};
      dynamic_cast<control*>(data_->digits[~1_i].get())->mouse_move += {*this, &lcd_label::on_digit_mouse_move};
      dynamic_cast<control*>(data_->digits[~1_i].get())->mouse_up += {*this, &lcd_label::on_digit_mouse_up};
    }
