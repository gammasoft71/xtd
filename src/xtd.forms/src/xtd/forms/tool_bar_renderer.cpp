#include "../../../include/xtd/forms/style_sheets/tool_bar.hpp"
...
void tool_bar_renderer::draw_tool_bar(const xtd::forms::style_sheets::style_sheet& style_sheet, xtd::drawing::graphics& graphics, const xtd::drawing::rectangle& bounds, xtd::forms::visual_styles::form_state form_state, const std::optional<xtd::drawing::color>& back_color, const std::optional<border_style>& border, xtd::forms::border_sides sides) {
  ...
  switch (form_state) {
    ...
    case control_state::pressed: current_style_sheet = style_sheet.tool_bar(style_sheets::pseudo_state::standard | pseudo_state::pressed); break;
    case control_state::disabled: current_style_sheet = style_sheet.tool_bar(style_sheets::pseudo_state::standard | pseudo_state::disabled); break;
  }
  
  if (back_color) current_style_sheet.background_color(back_color.value());
  if (border) {
    if (current_style_sheet.background_color() == color::transparent) current_style_sheet.border_color(xtd::forms::style_sheets::border_color(style_sheet::current_style_sheet().tool_bar().color()));
