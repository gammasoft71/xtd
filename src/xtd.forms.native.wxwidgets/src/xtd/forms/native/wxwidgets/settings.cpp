...
class file_settings {
public:
  file_settings(const string& product_name, const string& company_name) noexcept : name_ {path {getenv("HOME")} / path {".config"} / path {company_name} / path {product_name + ".conf"}} {
    create_directory(name_.parent_path());
    reload();
  }
