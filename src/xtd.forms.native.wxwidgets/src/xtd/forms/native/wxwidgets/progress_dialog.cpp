...
namespace {
  ...
  public:
    wx_progress_dialog(const string& title, const string& message, int32 maximum = 100, wxWindow* parent = nullptr, int32 style = wxPD_APP_MODAL | wxPD_AUTO_HIDE) : wxProgressDialog(convert_string::to_wstring(title).chars().c_str(), convert_string::to_wstring(message).chars().c_str(), maximum, parent, style) {
    ...
    void message(const string& message) {
      if (!timer_marquee.IsRunning()) Update(value_, convert_string::to_wstring(message).chars().c_str());
      else {
        timer_marquee.Stop();
        Pulse(convert_string::to_wstring(message).chars().c_str());
