...
void tab_page::image_index(intptr control, xtd::usize image_index) {
  ...
  if (note_book && note_book->GetImageList() && image_index < static_cast<xtd::usize>(note_book->GetImageList()->GetImageCount())) note_book->SetPageImage(note_book->FindPage(page), static_cast<int32>(image_index));
}
