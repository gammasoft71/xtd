...
namespace {
  ...
  wxTextEntryDialog* create_text_entry_dialog(intptr control, const string& text, const string& message, string& value, int32 character_casing, bool multiline, bool use_system_password_char) {
    ...
    wxTextEntryDialog* text_entry_dialog = new wxTextEntryDialog(control == 0 ? nullptr : reinterpret_cast<control_handler*>(control)->control(), message == "" ? L" " : convert_string::to_wstring(message).chars().c_str(), convert_string::to_wstring(text).chars().c_str(), convert_string::to_wstring(value).chars().c_str(), style);
