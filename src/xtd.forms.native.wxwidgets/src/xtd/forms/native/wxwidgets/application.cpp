...
void application::initialize() {
  ...
  [[maybe_unused]] auto __ = drawing::native::toolkit::initialize();
...
void application::register_wnd_proc(const delegate<intptr(intptr, uint32, intptr, intptr, intptr)>& wnd_proc) {
  initialize(); // Must be first
...
void application::restart(bool exit_after_restart) {
  ...
  for (auto index = 1_z; index < environment::get_command_line_args().length(); ++index)
    command_line_args += string::format(" {}", (environment::get_command_line_args()[index].chars().find(" ") ? environment::get_command_line_args()[index].quoted() : environment::get_command_line_args()[index]));
