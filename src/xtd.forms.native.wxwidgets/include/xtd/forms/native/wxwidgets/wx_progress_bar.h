...
namespace xtd {
  namespace forms {
    namespace native {
      ...
      private:
        explicit wx_progress_bar(const forms::create_params& create_params) {
          if (!create_params.parent()) throw xtd::argument_exception("control must have a parent"_t, current_stack_frame_);
          control_handler::create<wxGauge>(reinterpret_cast<control_handler*>(create_params.parent())->main_control(), wxID_ANY, 100, wxPoint(create_params.x(), create_params.y()), wxDefaultSize, style_to_wx_style(create_params.style(), create_params.ex_style()));
