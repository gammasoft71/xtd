    ...
    namespace native {
      ...
      public:
        explicit wx_form(const forms::create_params& create_params) {
          ...
          if ((create_params.ex_style() & WS_EX_MODALWINDOW) == WS_EX_MODALWINDOW) control_handler::create<wxDialog>(create_params.parent() ? (reinterpret_cast<control_handler*>(create_params.parent())->control()) : nullptr, wxID_ANY, wxString(xtd::convert_string::to_wstring(create_params.caption())), location, size, form_style_to_wx_style(create_params.style(), create_params.ex_style(), create_params.class_style(), create_params.parent()));
