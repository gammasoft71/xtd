...
class type;
  ...
  public:
    ...
    static bool equals(const object& object_a, const object& object_b) {return object_a.equals(object_b);}
    
    /// @brief Serves as a hash function for a particular type.
    /// @return int32 _t A hash code for the current object.
    virtual int32_t get_hash_code() const {return static_cast<int32_t>(reinterpret_cast<int64_t>(this) & 0x00000000FFFFFFFF) ^ static_cast<int32_t>((reinterpret_cast<int64_t>(this) >> 32) & 0x00000000FFFFFFFF);}
    ...
    template<typename T>
    std::shared_ptr<object> memberwise_clone() const {return std::make_shared<T>(dynamic_cast<const T&>(*this));}
    ...
    static bool reference_equals(const object& object_a, const object& object_b) {return &object_a == &object_b;}
    ...
    virtual std::string to_string() const {return strings::full_class_name(*this);}
  };
}
