...
string file_system::get_full_path(const string& relative_path) {
  auto directories = native::serenityos::strings::split(relative_path, {path::directory_separator_char()}, std::numeric_limits<size_t>::max(), true);
  ...
  for (auto item : directories) {
    if (item == ".." && native::serenityos::strings::last_index_of(full_path, path::directory_separator_char()) != full_path.npos)
      full_path = native::serenityos::strings::remove(full_path, native::serenityos::strings::last_index_of(full_path, path::directory_separator_char()));
    else if (item != ".")
  ...
  auto index = native::serenityos::strings::last_index_of(full_path, "/./");
  while (index != full_path.npos) {
    full_path = native::serenityos::strings::remove(full_path, index, 2);
    index = native::serenityos::strings::last_index_of(full_path, "/./");
  }
...
int32_t file_system::set_attributes(const std::string& path, int32_t attributes) {
  ...
  // The other attributes can be modified under serenityos.
  
  return chmod(path.c_str(), s.st_mode);
}

int32_t file_system::set_creation_time(const std::string& path, time_t creation_time) {
  // There is no creation time on serenityos so we update the last modification time instead...
  auto times = utimbuf {};
