...
intmax_t unnamed_semaphore::create(std::int32_t initial_count, std::int32_t maximum_count) {
...
void unnamed_semaphore::destroy(std::intmax_t handle) {
  if (reinterpret_cast<sem_t*>(handle) == SEM_FAILED) return;
  semaphore_destroy(current_task(), static_cast<semaphore_t>(handle));
}

bool unnamed_semaphore::signal(std::intmax_t handle, std::int32_t release_count, std::int32_t& previous_count, bool& io_error) {
  io_error = false;
...
uint32_t unnamed_semaphore::wait(std::intmax_t handle, std::int32_t milliseconds_timeout) {
