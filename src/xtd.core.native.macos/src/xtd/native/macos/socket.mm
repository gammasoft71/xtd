...
int32_t socket::connect(std::intmax_t handle, const std::vector<std::uint8_t>& socket_address) {
  std::swap(const_cast<std::vector<std::uint8_t>&>(socket_address)[0], const_cast<std::vector<std::uint8_t>&>(socket_address)[1]);
...
intmax_t socket::create(std::int32_t address_family, std::int32_t socket_type, std::int32_t protocol_type) {
  return static_cast<std::intmax_t>(::socket(address_family_to_native(address_family), socket_type_to_native(socket_type), protocol_type_to_native(protocol_type)));
}

int32_t socket::destroy(std::intmax_t handle) {
  return ::close(static_cast<int32_t>(handle));
}

std::string socket::socket_error_to_string(std::int32_t socket_error) {
...
size_t socket::get_available(std::intmax_t handle) {
...
int32_t socket::get_raw_socket_option(std::intmax_t handle, std::int32_t socket_option_level, std::int32_t socket_option_name, std::intmax_t option, size_t& option_length) {
...
int32_t socket::get_socket_option(std::intmax_t handle, std::int32_t socket_option_level, std::int32_t socket_option_name, std::intmax_t option, size_t& option_length) {
  ...
  if (socket_option_level == SOCKET_OPTION_LEVEL_SOCKET && (socket_option_name == SOCKET_OPTION_NAME_SEND_TIMEOUT || socket_option_name == SOCKET_OPTION_NAME_RECEIVE_TIMEOUT)) {
    ...
    auto result = ::getsockopt(static_cast<int32_t>(handle), socket_option_level_to_native(socket_option_level), socket_option_name_to_native(socket_option_name), &timeout, reinterpret_cast<socklen_t*>(&option_length));
...
int32_t socket::get_socket_linger_option(std::intmax_t handle, bool& enabled, std::uint32_t& linger_time) {
...
int32_t socket::get_socket_multicast_option(std::intmax_t handle, std::int32_t socket_option_name, std::uint32_t& multicast_address, std::uint32_t& interface_index) {
  struct multicast {
    std::uint32_t multicast_address;
    std::uint32_t interface_index;
  } m;
  auto multicast_size = size_t {0};
  auto result = getsockopt(static_cast<int32_t>(handle), IPPROTO_IP, socket_option_name_to_native(socket_option_name), &m, reinterpret_cast<socklen_t*>(&multicast_size));
  if (result == 0) {
...
int32_t socket::get_socket_ip_v6_multicast_option(std::intmax_t handle, std::int32_t socket_option_name, std::vector<std::uint8_t>& multicast_address, std::uint64_t& interface_index) {
  struct multicast {
    std::uint8_t multicast_address[16];
    std::uint64_t interface_index;
  } m;
  auto multicast_size = size_t {0};
  auto result = getsockopt(static_cast<int32_t>(handle), IPPROTO_IP, socket_option_name_to_native(socket_option_name), &m, reinterpret_cast<socklen_t*>(&multicast_size));
  if (result == 0) {
...
int32_t socket::io_control(std::intmax_t handle, std::int32_t io_control, std::vector<std::uint8_t>& option_in_value, std::vector<std::uint8_t>& option_out_value) {
  // Not implemented
  errno = ENOTSUP;
  return -1;
}

int32_t socket::listen(std::intmax_t handle, size_t backlog) {
...
int32_t socket::poll(std::intmax_t handle, std::int32_t microseconds, std::int32_t mode) {
...
int32_t socket::receive(std::intmax_t handle, std::vector<std::uint8_t>& buffer, size_t offset, size_t size, std::int32_t flags) {
...
int32_t socket::receive_from(std::intmax_t handle, std::vector<std::uint8_t>& buffer, size_t offset, size_t size, std::int32_t flags, std::vector<std::uint8_t>& socket_address) {
  std::swap(socket_address[0], socket_address[1]);
  auto address_length = static_cast<socklen_t>(socket_address.size());
...
int32_t socket::select(std::vector<std::intmax_t>& check_read, std::vector<std::intmax_t>& check_write, std::vector<std::intmax_t>& check_error, std::int32_t microseconds) {
  auto nfds = size_t {0};
...
int32_t socket::send(std::intmax_t handle, const std::vector<std::uint8_t>& buffer, size_t offset, size_t size, std::int32_t flags) {
  return static_cast<int32_t>(::send(static_cast<int32_t>(handle), &buffer.data()[offset], size, flags));
}

int32_t socket::send_to(std::intmax_t handle, const std::vector<std::uint8_t>& buffer, size_t offset, size_t size, std::int32_t flags, const std::vector<std::uint8_t>& socket_address) {
  std::swap(const_cast<std::vector<std::uint8_t>&>(socket_address)[0], const_cast<std::vector<std::uint8_t>&>(socket_address)[1]);
...
int32_t socket::set_blocking(std::intmax_t handle, bool blocking) {
  auto result = -1;
...
int32_t socket::set_raw_socket_option(std::intmax_t handle, std::int32_t socket_option_level, std::int32_t socket_option_name, std::intmax_t option, size_t option_length) {
...
int32_t socket::set_socket_option(std::intmax_t handle, std::int32_t socket_option_level, std::int32_t socket_option_name, std::intmax_t option, size_t option_length) {
  if (socket_option_name_to_native(socket_option_name) == -1) {
    errno = ENOTSUP;
    return -1;
  }
  if (socket_option_level == SOCKET_OPTION_LEVEL_SOCKET && (socket_option_name == SOCKET_OPTION_NAME_SEND_TIMEOUT || socket_option_name == SOCKET_OPTION_NAME_RECEIVE_TIMEOUT)) {
    auto timeout = timeval {*reinterpret_cast<const std::int32_t*>(option) / 1000, * reinterpret_cast<const std::int32_t*>(option) % 1000 * 1000};
...
int32_t socket::set_socket_linger_option(std::intmax_t handle, bool enabled, std::uint32_t linger_time) {
  auto l = linger {static_cast<int32_t>(enabled), static_cast<int32_t>(linger_time)};
  return setsockopt(static_cast<int32_t>(handle), SOL_SOCKET, SO_LINGER, &l, static_cast<socklen_t>(sizeof(linger)));
}

int32_t socket::set_socket_multicast_option(std::intmax_t handle, std::int32_t socket_option_name, std::uint32_t multicast_address, std::uint32_t interface_index) {
  struct multicast {
    std::uint32_t multicast_address;
    std::uint32_t interface_index;
...
int32_t socket::set_socket_ip_v6_multicast_option(std::intmax_t handle, std::int32_t socket_option_name, const std::vector<std::uint8_t>& multicast_address, std::uint64_t interface_index) {
  struct multicast {
    std::uint8_t multicast_address[16];
    std::uint64_t interface_index;
  } m;
...
int32_t socket::shutdown(std::intmax_t handle, std::int32_t how) {
  auto result = ::shutdown(static_cast<int32_t>(handle), how);
