#define __XTD_CORE_NATIVE_LIBRARY__
#include <xtd/native/critical_section>
#include "../../../../include/xtd/native/macos/mutex.hpp"
#undef __XTD_CORE_NATIVE_LIBRARY__
#include <pthread.h>

using namespace xtd::native;

intmax_t critical_section::create() {
  auto handle = new pthread_mutex_t;
  auto mutex_attribute = pthread_mutexattr_t {};
  if (pthread_mutexattr_init(&mutex_attribute) != 0 ||
    pthread_mutexattr_settype(&mutex_attribute, PTHREAD_MUTEX_RECURSIVE) != 0 ||
    pthread_mutex_init(handle, &mutex_attribute) != 0) {
    delete handle;
    return reinterpret_cast<std::intmax_t>(MUTEX_FAILED);
  }
  return reinterpret_cast<std::intmax_t>(handle);
}

void critical_section::destroy(std::intmax_t handle) {
  if (handle == reinterpret_cast<std::intmax_t>(MUTEX_FAILED)) return;
  pthread_mutex_destroy(reinterpret_cast<pthread_mutex_t*>(handle));
  delete reinterpret_cast<pthread_mutex_t*>(handle);
}

void critical_section::enter(std::intmax_t handle) {
  if (handle == reinterpret_cast<std::intmax_t>(MUTEX_FAILED)) return;
  pthread_mutex_lock(reinterpret_cast<pthread_mutex_t*>(handle));
}

bool critical_section::try_enter(std::intmax_t handle) {
  if (handle == reinterpret_cast<std::intmax_t>(MUTEX_FAILED)) return false;
  return pthread_mutex_trylock(reinterpret_cast<pthread_mutex_t*>(handle)) == 0;
}

void critical_section::leave(std::intmax_t handle) {
  if (handle == reinterpret_cast<std::intmax_t>(MUTEX_FAILED)) return;
  pthread_mutex_unlock(reinterpret_cast<pthread_mutex_t*>(handle));
}
