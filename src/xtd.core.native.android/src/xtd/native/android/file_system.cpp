...
string file_system::get_full_path(const string& relative_path) {
  auto directories = native::android::strings::split(relative_path, {path::directory_separator_char()}, std::numeric_limits<size_t>::max(), true);
  ...
  for (auto item : directories) {
    if (item == ".." && native::android::strings::last_index_of(full_path, path::directory_separator_char()) != full_path.npos)
      full_path = native::android::strings::remove(full_path, native::android::strings::last_index_of(full_path, path::directory_separator_char()));
    else if (item != ".")
  ...
  auto index = native::android::strings::last_index_of(full_path, "/./");
  while (index != full_path.npos) {
    full_path = native::android::strings::remove(full_path, index, 2);
    index = native::android::strings::last_index_of(full_path, "/./");
  }
