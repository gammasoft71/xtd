...
namespace {
  ...
  public:
    wx_radio_button(wxWindow *parent, const wxString& title, const wxPoint& pos, const wxSize& size, long style, bool push_like) {
      if (!push_like)
        this->control_handler::create<wxRadioButton>(parent, wxID_ANY, title, pos, size, style);
      else
