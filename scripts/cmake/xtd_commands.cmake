...
macro(target_type TYPE)
  ...
  include(${CMAKE_CURRENT_SOURCE_DIR}/properties/resource_strings.cmake OPTIONAL)
  if (EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/properties/resource_strings.cmake)
    set(CMAKE_RESOURCE_STRINGS properties/resource_strings.cmake)
  ...
  if (EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/locale")
    ...
    if (APPLE AND ("${TYPE}" STREQUAL "GUI_APPLICATION" OR "${TYPE}" STREQUAL "C_GUI_APPLICATION"))
      ...
      file(COPY ${CMAKE_CURRENT_SOURCE_DIR}/locale DESTINATION ${CMAKE_CURRENT_BINARY_DIR}/resources)
      file(COPY ${CMAKE_CURRENT_SOURCE_DIR}/locale DESTINATION ${CMAKE_CURRENT_BINARY_DIR}/Debug/resources)
      file(COPY ${CMAKE_CURRENT_SOURCE_DIR}/locale DESTINATION DESTINATION ${CMAKE_CURRENT_BINARY_DIR}/Release/resources)
    endif ()
  endif ()
  ...
  if ("${TYPE}" STREQUAL "CONSOLE_APPLICATION" OR "${TYPE}" STREQUAL "C_CONSOLE_APPLICATION")
    ...
    if ("${TYPE}" STREQUAL "C_CONSOLE_APPLICATION")
      set(PROJECT_TYPE_REFERENCE xtd_c)
    else()
      set(PROJECT_TYPE_REFERENCE xtd)
    endif ()
  ...
  if (APPLE AND "${TYPE}" STREQUAL "GUI_APPLICATION")
    set_target_properties(${PROJECT_NAME} PROPERTIES XCODE_ATTRIBUTE_PRODUCT_BUNDLE_IDENTIFIER "${ASSEMBLY_IDENTIFIER}")
  endif ()
...
##  add_sources(my_project.cpp)
##
##  assembly_copyright("© 2026 by Gammasoft.")
##
##  target_type(GUI_APPLICATION)
## ```
...
macro(write_resources)
  ...
  foreach (RESOURCE_LINE ${PROJECT_RESOURCES})
    ...
    elseif ("${TARGET_TYPE}" STREQUAL "GUI_APPLICATION" OR "${TARGET_TYPE}" STREQUAL "C_GUI_APPLICATION" OR "${TARGET_TYPE}" STREQUAL "CONSOLE_APPLICATION" OR "${TARGET_TYPE}" STREQUAL "C_CONSOLE_APPLICATION" OR "${TARGET_TYPE}" STREQUAL "TEST_APPLICATION" OR "${TARGET_TYPE}" STREQUAL "C_TEST_APPLICATION")
      if (CMAKE_BUILD_TYPE)
        get_filename_component(LAST_DIR_NAME "${RESOURCES_DIRECTORY_ROOT}" NAME)
        if (APPLE OR LAST_DIR_NAME STREQUAL "${CMAKE_BUILD_TYPE}")
          configure_file(${FILE} ${RESOURCES_DIRECTORY_ROOT}/resources/${FILENAME} COPYONLY)
        else ()
          configure_file(${FILE} ${RESOURCES_DIRECTORY_ROOT}/${CMAKE_BUILD_TYPE}/resources/${FILENAME} COPYONLY)
        endif ()
      else ()
        configure_file(${FILE} ${RESOURCES_DIRECTORY_ROOT}/resources/${FILENAME} COPYONLY)
...
macro(write_macos_target_informations_file)
  ...
  # TODO : Use "assembly_identifier" or if empty "com.${ASSEMBLY_COMPANY}.${ASSEMBLY_PRODUCT}" or if emty fallback like "com.gammasoft.${PROJECT_NAME}".
  string(REGEX REPLACE "[^A-Za-z0-9.-]" "-" SAFE_PROJECT_NAME "${PROJECT_NAME}")
  string(TOLOWER "${SAFE_PROJECT_NAME}" SAFE_PROJECT_NAME)
  set(ASSEMBLY_IDENTIFIER "xtd.${SAFE_PROJECT_NAME}")
  set(MACOSX_BUNDLE_GUI_IDENTIFIER "${ASSEMBLY_IDENTIFIER}")
...
macro(write_windows_target_informations_file)
  ...
  elseif ("${FILE_VERSION_STR}" STREQUAL "*.0")
    set(FILE_VERSION_STR "${CMAKE_PROJECT_VERSION}.0")
...
macro(write_resources_file_header)
  ...
  set(TEXT_EXTENSIONS ".md" ".txt" ".text")
  foreach(RESOURCE_LINE ${PROJECT_RESOURCES})
    split_resource(${RESOURCE_LINE} NAME FILE)
    get_filename_component(FILENAME ${FILE} NAME)
