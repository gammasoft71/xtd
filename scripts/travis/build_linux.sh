...
cmake -DCMAKE_BUILD_TYPE=Debug -DXTD_ENABLE_TESTS=ON -DXTD_DISABLE_FOMRS_TESTS=ON -DXTD_ENABLE_XTD_COMMAND_LINE=OFF -DCMAKE_CXX_COMPILER=g++-9 -DCMAKE_INSTALL_PREFIX=~/local ..
if [ $? -ne 0 ]; then exit -1; fi
cmake --build . -- -j $(nproc)
if [ $? -ne 0 ]; then exit -1; fi
