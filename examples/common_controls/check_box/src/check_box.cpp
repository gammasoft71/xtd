...
namespace examples {
  ...
  public:
    form1() {
      ...
      this->check_box3.location({10, 70});
      this->check_box3.text("indeterminate");
      this->check_box3.check_state_changed += [&](const control& sender, const event_args& e) {
        this->label1.text(strings::format("check_box3.check_state() = {}", this->check_box3.check_state()));
      };
      
      this->label1.parent(*this);
      this->label1.auto_size(true);
      this->label1.location({10, 150});
      this->label1.text(strings::format("check_box3.check_state() = {}", this->check_box3.check_state()));
    }
