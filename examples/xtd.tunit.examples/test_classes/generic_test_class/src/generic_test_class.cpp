...
template<>
size_t get_max_value<xtd::sbyte>() {return 127;}
...
namespace unit_tests {
  ...
  test_class_attribute<test<xtd::sbyte>> test_int8_t_class_attr {"test<sbyte>"};
  ...
  public:
    void test_method_(test_max_value) {
      static std::map<std::string, size_t> expected_max_values = {{typeid(bool).name(), std::numeric_limits<bool>::max()}, {typeid(int).name(), std::numeric_limits<int>::max()}, {typeid(long int).name(), std::numeric_limits<long int>::max()}, {typeid(xtd::sbyte).name(), std::numeric_limits<xtd::sbyte>::max()}};
