...
struct foo2 {
  ...
  auto operator ==(const foo2& f) const noexcept -> bool {return value == f.value;}
  auto operator <(const foo2& f) const noexcept -> bool {return value < f.value;}
  auto operator >(const foo2& f) const noexcept -> bool {return value > f.value;}
};

template<>
auto xtd::to_string(const foo2& value, const string& fmt, const std::locale& loc) -> string {return xtd::int32_object {value.value}.to_string(fmt, loc);}

template<>
struct std::hash<foo2> {
  auto operator()(const foo2& value) const noexcept -> xtd::size {return xtd::hash_code::combine(value.value);}
};

template<typename type_t>
auto get_any_info(const type_t& value) -> string {
