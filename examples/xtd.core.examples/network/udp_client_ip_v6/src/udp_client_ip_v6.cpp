...
auto main()->int {
  ...
  auto server = thread {[&] {
    auto udp = udp_client {ip_end_point {ip_address::ip_v6_any, 9400}};
    while (!terminate_app) {
  ...
  auto client = thread {[&] {
    ...
    while (!terminate_app) {
      ...
      udp.send(vector<unsigned char>(str.begin(), str.end()), str.size(), ip_end_point(ip_address::ip_v6_loopback, 9400));
      sleep_for(50_ms);
    }
    
    udp.send(vector<unsigned char> {0xFF}, 1, ip_end_point(ip_address::ip_v6_loopback, 9400));
  }};
