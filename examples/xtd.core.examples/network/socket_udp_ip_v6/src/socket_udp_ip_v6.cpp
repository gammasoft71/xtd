...
auto main() -> int {
  ...
  auto client = thread{[&] {
    ...
    while (!terminate_app) {
      ...
      client_socket.send_to(array<byte>(str.begin(), str.end()), ip_end_point {ip_address::ip_v6_loopback, 9400});
      thread::sleep(50_ms);
    }
    
    client_socket.send_to(array<byte> {0xFF}, ip_end_point {ip_address::ip_v6_loopback, 9400});
  }};
