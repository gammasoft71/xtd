...
namespace image_converter_example {
  ...
  private:
    void on_input_change() {
      ...
      if (effect_choice.selected_item() == "bi-tonal") adjusted_image = image_converter::bi_tonal(original_image(), bi_tonal_threshold_track_bar.value(), bi_tonal_upper_color_picker.color(), bi_tonal_lower_color_picker.color());
    ...
    label percent_opacity_label = label::create(opacity_panel, "Percent", {10, 33}, {70, 33});
    ...
    check_box resize_maintain_aspect_ratio_check_box = check_box::create(resize_panel, "Maintain aspect ratio", check_state::checked, {420, 33}, {150, 23});
