  ...
  public:
    void on_paint(paint_event_args& e) override {
      auto accentuate_color = [](const color& c, float percent) {
        if (application::dark_mode_enabled()) return control_paint::light(c, percent);
        return control_paint::dark(c, percent);
      };
      auto background_color = back_color_.value_or(xtd::forms::theme_colors::current_theme().control());
      auto foreground_color = fore_color_.value_or(xtd::forms::theme_colors::current_theme().control_text());
      ...
      if (state() == xtd::forms::visual_styles::push_button_state::hot || state() == xtd::forms::visual_styles::push_button_state::default_state) {
