...
public:
  ...
  form1() {
    ...
    themes_choice.items().add("current theme");
    auto names = application::style_sheet_names();
    names.sort();
    themes_choice.items().add_range(names);
...
private:
  void on_menu_click(object& sender, const event_args& e) {
    ...
    if (main_menu1.menu_items()[3].get().menu_items()[4].get() == sender) about_box::show(*this, "Shows some controls with theme", "Themes example", xtd::drawing::system_icons::xtd_logo(), environment::version().to_string(2), environment::version().to_string(), "Copyright (c) 2024 Gammasoft.", "https://gammasoft71.github.io/xtd", "xtd");
  }
