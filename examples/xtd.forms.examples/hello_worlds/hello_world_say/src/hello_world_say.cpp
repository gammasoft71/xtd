...
public:
  form1() {
    if (environment::os_version().is_windows_platform()) file::write_all_text((temp_directory_path()/"say.cmd").string(), "@echo Set Speaker=CreateObject(\"sapi.spvoice\") > %TEMP%\\say_.vbs\n@echo Speaker.Speak %* >> %TEMP%\\say_.vbs\n@%TEMP%\\say_.vbs");
    else if (environment::os_version().is_macos_platform()) file::write_all_text((temp_directory_path()/"say.cmd").string(), "say $*");
