...
public:
  form1() {
    ...
    timer1.enabled(true);
    timer1.tick += [&] {
      sixteen_segment_display1.segment_style(segment_styles[(counter / crawler.size()) % segment_styles.size()]);
      sixteen_segment_display1.value(crawler[counter++ % crawler.size()]);
    };
