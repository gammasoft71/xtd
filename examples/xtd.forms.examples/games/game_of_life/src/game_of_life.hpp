...
namespace game_of_life {
  class form_game_of_life : public form {
    using figure_delegate = delegate<void()>;
  public:
    static auto main() {
      application::run(game_of_life::form_game_of_life());
    }
    ...
    form_game_of_life() {
      ...
      label_iterations_.font(drawing::font(font_family::generic_monospace(), label_iterations_.font().size()));
      label_iterations_.text_align(content_alignment::middle_right);
      label_iterations_.text(string::format("Iterations : {}", iterations_));
      label_iterations_.width(150);
      label_iterations_.location({555, 12});
      label_iterations_.anchor(anchor_styles::top | anchor_styles::right);
      
      label_zoom_.parent(*this);
      label_zoom_.location({10, 45});
      label_zoom_.text(string::format("Zoom : {}", zoom_));
      ...
      track_bar_zoom_.tick_style(tick_style::none);
      ...
      label_speed_.anchor(anchor_styles::top | anchor_styles::right);
      label_speed_.text(string::format("Speed : {}", speed_));
      ...
      track_bar_speed_.tick_style(tick_style::none);
      ...
      track_bar_speed_.anchor(anchor_styles::top | anchor_styles::right);
      
      panel_grid_.parent(*this);
      panel_grid_.back_color(empty_color_);
      panel_grid_.border_style(forms::border_style::fixed_single);
      panel_grid_.location({10, 75});
      panel_grid_.size({695, 395});
      panel_grid_.anchor(anchor_styles::top | anchor_styles::left | anchor_styles::bottom | anchor_styles::right);
      panel_grid_.double_buffered(true);
      
      panel_grid_.mouse_down += delegate_(object & sender, const mouse_event_args & e) {
        ...
        panel_grid_.invalidate(rectangle(e.location().x / zoom_ * zoom_, e.location().y / zoom_ * zoom_, zoom_, zoom_), false);
      };
      
      panel_grid_.mouse_move += delegate_(object & sender, const mouse_event_args & e) {
      ...
      panel_grid_.paint += delegate_(object & sender, paint_event_args & e) {
          ...
          for (auto y = 0; y < panel_grid_.client_size().height; y += zoom_)
            for (auto x = 0; x < panel_grid_.client_size().width; x += zoom_)
              if (grid_.cells()[offset_y_ + y / zoom_][offset_x_ + x / zoom_] != cell::empty)
                e.graphics().fill_rectangle(solid_brush(populated_color_), x, y, zoom_, zoom_);
        if (zoom_ > 3) {
          for (auto index = 0; index < panel_grid_.client_size().width; index += zoom_)
            e.graphics().fill_rectangle(solid_brush(line_color_), index, 0, 1, panel_grid_.client_size().height);
          for (auto index = 0; index < panel_grid_.client_size().height; index += zoom_)
            e.graphics().fill_rectangle(solid_brush(line_color_), 0, index, panel_grid_.client_size().width, 1);
        }
      };
      ...
      thread_run_ = thread {[&] {
        while (!closed_) {
          if (!run_enabled_)
            thread::sleep(std::chrono::milliseconds(200));
          else {
            next();
            ...
            if (elapsedTime < std::chrono::milliseconds(interval_milliseconds_)) thread::sleep(std::chrono::milliseconds(interval_milliseconds_) - elapsedTime);
            else thread::yield();
  ...
  private:
    ...
    void random() {
      ...
      for (auto counter = 0; counter < max; counter++) {
        ...
        panel_grid_.invalidate(rectangle(x * zoom_, y * zoom_, zoom_, zoom_), false);
      }
    }
    ...
    choice choice_figures_;
    label label_iterations_;
    label label_zoom_;
    track_bar track_bar_zoom_;
    label label_speed_;
    track_bar track_bar_speed_;
    panel panel_grid_;
    thread thread_run_;
    color line_color_ = system_colors::window_text();
    color empty_color_ = system_colors::window();
    color populated_color_ = system_colors::window_text();
  };
}
