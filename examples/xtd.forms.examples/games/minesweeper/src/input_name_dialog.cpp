...
minesweeper::level input_name_dialog::level() const {
  return level_;
}

void input_name_dialog::level(minesweeper::level level) {
  level_ = level;
  message_label_.text(xtd::strings::format("You have the fastest time\nfor {} level.\nPlease type your name."_t, level_to_string()));
}

std::string input_name_dialog::gammer_name() const {
  return gammer_name_;
}

void input_name_dialog::gammer_name(const std::string& gammer_name) {
  gammer_name_ = gammer_name;
  name_text_box_.text(gammer_name_);
}

std::string input_name_dialog::level_to_string() {
