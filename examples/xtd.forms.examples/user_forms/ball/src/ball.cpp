...
main_form::main_form() {
  ...
  // Mouse tracking
  main_panel_.mouse_down += [&](object& sender, const mouse_event_args& e) {
  ...
  main_panel_.mouse_up += [&](object& sender, const mouse_event_args& e) {
    if (!is_dragging || e.button() == mouse_buttons::right) return;
    is_dragging = false;
  ...
  main_panel_.mouse_move += [&](object& sender, const mouse_event_args& e) {
  ...
  animation_timer_.interval(16_ms); // ~60 FPS
  animation_timer_.tick += [&] {
