...
namespace examples {
  ...
  protected:
    void on_paint(paint_event_args& e) override {
      ...
      auto message = strings::to_u32string(text());
      for (auto i = 0u; i < message.length(); i++) {
        auto index = (step + i) % sins.size();
        e.graphics().draw_string(strings::format("{}", message[i]), font(), solid_brush {color::from_hsb(360.0f / sins.size() * index, 1.0f, 0.75f)}, point::subtract(pos, point(0, sins[index] * font().height() / 400)));
        pos.x(pos.x() + static_cast<int>(e.graphics().measure_string(strings::format("{}", message[i]), font()).width()));
      }
    }
