  ...
  protected:
    void on_paint(paint_event_args& e) override {
      ...
      for (auto i = 0U; i < text().length(); i++) {
        ...
        e.graphics().draw_string(format("{}", text()[i]), font(), solid_brush {color::from_hsb(360.0f / sins.size() * index, 1.0f, 0.75f)}, point::subtract(pos, point(0, sins[index] * font().height() / 400)));
        pos.x(pos.x() + static_cast<int>(e.graphics().measure_string(format("{}", text()[i]), font()).width()));
      }
    }
