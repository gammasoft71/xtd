  ...
  public:
    form1() {
      ...
      paint += [](object & sender, paint_event_args & e) {
        ...
        e.graphics().draw_string("Drawing with graphics", drawing::font("Arial", 34, font_style::regular), brushes::dark_cyan(), {20.0f, 20.0f, e.clip_rectangle().width() - 40.0f, e.clip_rectangle().height() - 40.0f});
        e.graphics().fill_ellipse(brushes::yellow(), e.clip_rectangle().width() - 150, 50, 100, 100);
        ...
        e.graphics().fill_polygon(brushes::brown(), vector<point> {{e.clip_rectangle().width() / 2, e.clip_rectangle().height() - 300}, {e.clip_rectangle().width() / 2 + 160, e.clip_rectangle().height() - 180}, {e.clip_rectangle().width() / 2 - 160, e.clip_rectangle().height() - 180},
        });
      };
    }
  };
