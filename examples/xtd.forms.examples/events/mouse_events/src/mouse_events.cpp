...
public:
  form1() {
    ...
    mouse_horizontal_wheel += [&](control& sender, const mouse_event_args& e) {
      trace::write_line("mouse_horizontal_wheel={{button={}, clicks={}, delta={}, location=[{}], modifier_keys=[{}]}}", e.button(), e.clicks(), e.delta(), e.location(), modifier_keys());
    };
    ...
    mouse_wheel += [&](control& sender, const mouse_event_args& e) {
      trace::write_line("mouse_wheel={{button={}, clicks={}, delta={}, location=[{}], modifier_keys=[{}]}}", e.button(), e.clicks(), e.delta(), e.location(), modifier_keys());
    };
  }
};
