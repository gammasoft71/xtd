...
public:
  main_form() {
    ...
    xtd_web_button.click += delegate_ {
      diagnostics::process::start(diagnostics::process_start_info {"https://gammasoft71.github.io/xtd"}.use_shell_execute(true));
    };
    ...
    open_home_folder_button.click += delegate_ {
      diagnostics::process::start(diagnostics::process_start_info {environment::get_folder_path(environment::special_folder::home)}.use_shell_execute(true));
    };
    ...
    open_readme_txt_file_button.click += delegate_ {
      ...
      diagnostics::process::start(diagnostics::process_start_info {path::combine(path::get_temp_path(), "readme.txt")}.use_shell_execute(true));
    };
    ...
    open_gammasoft_png_file_button.click += delegate_ {
      ...
      diagnostics::process::start(diagnostics::process_start_info {"gammasoft.png"}.working_directory(path::get_temp_path()).use_shell_execute(true));
    };
    ...
    open_calculator_button.click += delegate_ {
      if (environment::os_version().is_windows()) diagnostics::process::start(diagnostics::process_start_info {"calc"});
      else if (environment::os_version().is_macos()) diagnostics::process::start(diagnostics::process_start_info {"Calculator"}.use_shell_execute(true));
      else if (environment::os_version().is_linux()) diagnostics::process::start(diagnostics::process_start_info {"gnome-calculator"});
