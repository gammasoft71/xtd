...
namespace examples {
  ...
  public:
    ...
    main_form() {
      ...
      auto name = [](const country& c) {return strings::format("{} ({})", c.emoticon(), c.name());};
      ...
      informations.border_style(border_style::fixed_3d).text(strings::format("Name: {0}Emoticon: {0}Alpha code: {0}Numeric code:", '\n')).parent(*this).location({10, 45}).size({540, 125});
    }
    
  private:
    void on_button_click(control& sender, const event_args& e) {
      auto c = std::any_cast<country>(sender.tag());
      informations.text(strings::format("Name: {1}{0}Emoticon: {2}{0}Alpha code: {3} ({4}){0}Numeric code: {5:D3}", '\n', c.name(), c.emoticon(), c.alpha_2_code(), c.alpha_3_code(), c.numeric_code()));
    }
