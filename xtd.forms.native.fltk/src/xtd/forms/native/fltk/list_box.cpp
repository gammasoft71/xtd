...
void list_box::insert_item(intptr_t control, size_t index, const ustring& value) {
  if (control == 0) return;
  static_cast<Fl_Browser*>(reinterpret_cast<control_handler*>(control)->control())->insert(index, value.c_str());
...
size_t list_box::selected_index(intptr_t control) {
  ...
  return static_cast<Fl_Browser*>(reinterpret_cast<control_handler*>(control)->control())->value();
}

void list_box::selected_index(intptr_t control, size_t index) {
  if (control == 0) return;
  static_cast<Fl_Browser*>(reinterpret_cast<control_handler*>(control)->control())->value(index);
}

vector<size_t> list_box::selected_indices(intptr_t control) {
  if (control == 0) return {};
  vector<size_t> indices;
  for (int index =0 ; index < static_cast<Fl_Browser*>(reinterpret_cast<control_handler*>(control)->control())->size(); index++)
